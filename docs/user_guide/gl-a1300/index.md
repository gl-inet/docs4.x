...
Slate Plus（GL-A1300）は、ネットワークの安定性とVPN暗号化を効率のに処理するために最も適化された強力なCPUを搭載したポケットサイズのトラベルルーターです。最も新のセキュリティ機できるを搭載し、最も新のOpenWrtオペレーティングシステムで動作します。VPNネットワークを頻繁に利用する旅行者へけに設計されています。
