...
??? "対応モデル"
    ...
    **Footnote:**  
    ...
    1. WPA/WPA2/WPA3-Enterpriseネットワークに接続する場合、GL-BE10000 (Slate 7 Pro)、GL-MT3600BE (Beryl 7)、GL-BE3600 (Slate 7) はPEAP認証ネットワークのみ対応しています。その他のEAP方式（EAP-TLS、EAP-TTLS、EAP-MD5など）はサポートされていません。
    
    2. GL-MT6000 (Flint 2) と GL-MT3000 (Beryl AX) は、デフォルトファームウェアでは EAP ネットワークへの接続をサポートしていません。ただし、GL.iNet はこれらのモデル向けにネイティブ OpenWrt 24 ファームウェアを提供しており、これをインストールすると EAP ネットワークに接続できるようになります。詳細は [Download Center](https://dl.gl-inet.com/){target="_blank"} でモデルを検索し、OPENWRT 24 タブを確認してください。

    3. GL-SFT1200 (Opal) は、ファームウェア v4.8 以降で EAP ネットワークへの接続をサポートしています。
...
> **注意**: 以下の手順はあくまで参考です。実際のデバイスの LuCI ページをご参照ください。
...
7. 接続を確認します。
    ...
    ??? "WebGUI で接続を確認する"
        ...
        **注意**: LuCI の設定は WebGUI の設定と同期されないため、接続された IP、gateway など、リピーターインターフェースの詳細は WebGUI に表示されません。
