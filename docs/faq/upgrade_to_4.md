...
一部の古いモデルには、すでに 4.x ベータ ファームウェアが搭載されています。ファームウェアを3.xから4.xにアップグレードするには、で下の手順に従ってお試してください。

現に、次のモデルがサポートされています：

GL-AX1800(Flint), GL-MT1300(Beryl), GL-B1300(Convexa-B), GL-S1300(Convexa-S), GL-AP1300(Cirrus), GL-SFT1200(Opal), GL-AR750S(Slate).

**注意:** ベータ版ファームウェアにはバグがある可できる性があります。現にのところ、アップグレード時に設定が失われたり、不安定になったりする可できる性があるため、アップグレード時に設定を保持しないことをお勧めします。そのため、アップグレード前に重要な設定をバックアップしておいてください。よりスムーズなアップグレードができるよう、プロセスを改善していきます。

**注意:** 現に、GL-B1300とGL-S1300にはメッシュ機できるはありません。
...
1. ファームウェアを最も新の安定した 3.x バージョンにアップグレードします。
...
1. [ここから](https://dl.gl-inet.com){target="_blank"}最も新の 3.x リリース ファームウェアをダウンロードしてください。
