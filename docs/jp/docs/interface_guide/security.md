...
Access Control は、ファームウェア v4.7 以前では Local Access Control とも呼ばれ、ルーターの各種管理インターフェースへのアクセスを管理します。
...
Web や FTP などのルーターサービスを公開するには、それぞれのポートをルーター上で開放する必要があります。
