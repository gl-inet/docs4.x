# Guide d'utilisation du Flint (GL-AX1800)

## Aperçu du produit

Flint (GL-AX1800) est un routeur Wi‑Fi 6 double bande offrant une vitesse de connexion allant jusqu'à 600Mbps (2.4GHz) + 1200Mbps (5GHz). Sa vitesse de chiffrement VPN atteint 667Mbps et il peut être utilisé pour héberger des serveurs VPN. Flint convient parfaitement aux transmissions de données intensives, aux environnements avec de nombreux appareils connectés ou aux jeux nécessitant une latence ultra-faible.
...
## Configuration initiale du Flint
...
<small>(Cette vidéo utilise un autre routeur GL.iNet pour illustrer la configuration, mais les étapes sont identiques pour le Flint et les autres modèles de routeurs.)</small>
...
Configurez votre Flint à l'aide de l'une des méthodes de connexion Internet prises en charge : Ethernet, Repeater, Tethering et Cellular. Si vous souhaitez utiliser la fonctionnalité [Multi-WAN](../../interface_guide/multi-wan.md), configurez plus d'une connexion Internet.
...
=== "Tethering"
    ...
    1. Connectez votre appareil mobile au port USB du routeur à l'aide d'un câble USB.
...
Un VPN (réseau privé virtuel) crée un trafic sécurisé et chiffré entre votre appareil et le serveur VPN. Il ajoute une couche supplémentaire de confidentialité et de sécurité (client VPN) et permet d'accéder à un réseau distant (serveur VPN). Flint prend en charge OpenVPN, WireGuard et Tor.

=== "OpenVPN"

    Flint, comme les autres routeurs GL.iNet, prend en charge le protocole OpenVPN, qui offre une sécurité renforcée. Pour configurer OpenVPN, suivez ces tutoriels :
...
=== "WireGuard"
    ...
    Flint, comme les autres routeurs GL.iNet, prend en charge le protocole WireGuard, qui offre d'excellentes vitesses et une grande simplicité d'utilisation. Pour configurer WireGuard, suivez ces tutoriels :
...
=== "Port forwarding"
    ...
    Pour configurer le transfert de ports, consultez [Port Forwards](../../interface_guide/port_forwarding.md).
...
=== "Scheduled Tasks"
    ...
    * 5GHz Wi-Fi Status Schedule : définissez un calendrier pour contrôler la bande Wi‑Fi 5GHz, afin de mieux gérer la disponibilité du réseau et la consommation d'énergie.
