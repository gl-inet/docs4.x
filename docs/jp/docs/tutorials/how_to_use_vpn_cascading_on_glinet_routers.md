...
**VPN 1（ルー器をVPNサーバーとして）**：ルー器がVPNサーバーとして機能する場合、このサーバーに接続されたクライアントはデフォルトでルー器のISPネットワーク経由でインターネットにアクセスします。

**VPN 2（ルー器をVPNクライアントとして）**：ルー器はVPNクライアントとしても機能し、サードパーティのVPNサービスに接続します。

**VPNカスケード**：ルー器はVPN 1トンネルからVPN 2トンネルへトラフィックを転送し、VPN 1経由でルー器に接続された機器が、ルー器のISPネットワークではなくサードパーティのVPNサービス（VPN 2）経由でインターネットにアクセスできるようになります。
...
2. ルー器から設定ファイルをエクスポートし、VPN経由でルー器に接続するクライアント機器にアップロードします。
...
4. 接続が完たすると、**VPN Dashboard**ページが下図のようにのように表示され、ルー器はVPNサーバーとVPNクライアントの両方に設定されています。
    ...
    同じページのVPN Serverセクションに移動し、**Global Options**をクリックします。
...
5. VPNカスケードが有効になります。VPN経由でルー器に接続された機器は、ルー器のISPネットワークではなく、サードパーティのVPNサービス経由でインターネットにアクセスします。
...
4. Web管理パネルで、**VPN Dashboard**に移動します。VPNモードに基づいて以下の該当する指示に従ってください。

    ??? "Global Mode"
    
        VPNモードがGlobal Modeの場合、VPNクライアントが有効になると（下図のようにのように）、VPNカスケードが自動的に有効になります。
        
        VPN経由でルー器に接続された機器は、ルー器のISPネットワークではなく、サードパーティのVPNサービス経由でインターネットにアクセスします。
    ...
    ??? "Policy Mode"
        ...
        - **All Clients**：これには、すべてのLAN機器、Drop-in Gatewayからの機器、Guestネットワークからの機器、およびVPN経由でルー器に接続された機器が含まれます。
        
            すべての機器からのトラフィックに同じトンネルを適用する場合は、**All Clients**を選択して**Apply**をクリックします。

            ![all clients](https://static.gl.inet.com/docs/router/en/4/tutorials/vpn_cascading/all_clients.png){class="glboxshadow"}

        - **Specified Connection Types**：特定の方法（例：VPN経由で接続された機器など）でルー器に接続された機器を指定して、このトンネルルールを適用できます。
...
6. **接続テスト**：VPN経由でルー器に接続されたラップトップでブラウザを開き、[What Is My IP](https://whatismyipaddress.com/){target="_blank"}にアクセスしてパブリックIPを確認します。
