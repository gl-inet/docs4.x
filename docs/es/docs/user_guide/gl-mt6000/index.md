# Flint 2 (GL-MT6000) Guía del usuario

## Descripción general del producto

Flint 2 (GL-MT6000) es un router Wi-Fi 6 para hogar y oficina especialmente adecuado para transmisión intensiva de datos, conexión masiva de dispositivos o entornos de juego con latencia ultrabaja. Flint 2 ofrece velocidades WireGuard VPN de hasta 900 Mbps. Además, también admite redundancia de red avanzada, incluidas Multi-WAN, Failover y balanceo de carga, para garantizar una red ininterrumpida.
...
Vea a continuación el video de unboxing de Flint 2.
...
<small>(Este video utiliza un router GL.iNet diferente para mostrar la configuración, pero los pasos son los mismos para Flint 2 y otros modelos de router.)</small>
...
Configure su Flint 2 usando uno de los métodos de conexión a Internet admitidos: Ethernet, Repeater, Tethering y Cellular. Si desea usar la función [Multi-WAN](../../interface_guide/multi-wan.md), configure más de una conexión a Internet.
...
=== "WireGuard"
    ...
    Flint 2 (y otros routers GL.iNet) es compatible con el protocolo WireGuard, que ofrece gran velocidad y comodidad. Para configurar WireGuard, siga estos tutoriales:
...
=== "Parental Control"
    ...
    Parental Control está diseñado para ayudarle a administrar y controlar los dispositivos de sus hijos. Incluye limitar el tiempo de pantalla y restringir su acceso a determinados contenidos.

    Flint 2 ofrece dos opciones de control parental: la versión local desarrollada por GL.iNet y la versión integrada de Bark.
...
=== "Port forwarding"
    ...
    Para configurar el reenvío de puertos, consulte [Port Forwards](../../interface_guide/port_forwarding.md).
...
=== "Guest Network"
    ...
    La página Guest Network le permite establecer una subred dentro de los rangos de direcciones privadas IPv4 192.168.0.0/16, 172.16.0.0/12 o 10.0.0.0/8, especificar las direcciones IP de gateway y máscara de red, y configurar ajustes de seguridad como el aislamiento de AP para la red de invitados.
...
=== "Drop-in Gateway"
    ...
    Drop-in Gateway amplía la funcionalidad de su router principal con características que quizá no tenga, incluidas AdGuard Home, DNS cifrado y VPN.
...
=== "Scheduled Tasks"
    ...
    * Programación del estado del Wi-Fi de 5 GHz / 2,4 GHz: establezca un horario para controlar la banda Wi-Fi de 5 GHz / 2,4 GHz, lo que permite gestionar mejor la disponibilidad de la red y el consumo de energía.
...
Por la presente, GL TECHNOLOGIES (HONG KONG) LIMITED declara que el tipo de equipo radioeléctrico [AX6000 Wireless Router, GL-MT6000] es conforme con los requisitos esenciales y demás disposiciones pertinentes de la Directiva 2014/53/UE. El texto completo de la declaración UE de conformidad está disponible en la dirección Internet siguiente: [https://www.gl-inet.com/products/certificate](https://www.gl-inet.com/products/certificate){target="_blank"}.