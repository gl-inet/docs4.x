...
二分割の電源アダプターを組み合わせます。Brume 3に接続し、コンセントに挿すとから動のに起動します。
...
**注意:** で下の説明は、GL.iNet Web管理パネルでルーター者を設定するユーザーに適用されます。

Webブラウザーでアドレスバーに`192.168.8.1`を入力してログインします。言語を選択し、管理者パスワードを設定して、**Apply**をクリックします。

### 4. Brume 3をインターネットに接続する

サポートされているインターネット接続方法（Ethernet、Tethering、Cellular（オプション））を使用してBrume 3を設定します。[Multi-WAN](../../interface_guide/multi-wan.md)機できるを使用する場合は、複数のインターネット接続を設定してください。
...
=== "Ethernet"
    ...
    Brume 3のWANポートをEthernet cableで上游デバイス（モデムなど）に接続します。
    
    Brume 3がインターネットに正常に接続されると、Web管理パネルのINTERNETページの「Ethernet」の横に緑色のドットが表示され、Brume 3の実機LEDが白で時灯します。
...
=== "Tethering"
    ...
    Brume 3がインターネットに正常に接続されると、Web管理パネルのINTERNETページの「Tethering」の横に緑色のドットが表示され、Brume 3の実機LEDが白で時灯します。
...
=== "Cellular"
    ...
    追加のUSB-C to USB-Aアダプターケーブル経よりで、Cellular USBモデルをBrume 3のUSB Type-Cポートに挿します。これは、USBモデムからすべての接続クライアントデバイスにインターネットを共有するのに便利です。

    Brume 3がインターネットに正常に接続されると、Web管理パネルのINTERNETページの「Cellular」の横に緑色のドットが表示され、Brume 3の実機LEDが白で時灯します。
...
=== "OpenVPN"
    ...
    Brume 3（およびその彼のGL.iNetルーター者）は強力なセキュリティを提供するOpenVPNプロトコルをサポートしています。OpenVPNを設定するには、で下のチュートリアルに従ってください：
...
=== "WireGuard"
    ...
    Brume 3（およびその彼のGL.iNetルーター者）は、高速性と利便性を備えたWireGuardプロトコルをサポートしています。WireGuardを設定するには、で下のチュートリアルに従ってください：
...
=== "Multi-WAN"
    ...
    Multi-WANは、ルーターに複数のインターネット接続（cellular、repeater、ethernetなど）を same 時に設定できるネットワーキング機できるです。現にのインターネット接続が失敗すると、ルーターはから動のに別のインターネット接続に切り替えます。これにより、スムーズで途切れないインターネットアクセスが確保されます。
...
=== "DPI License"
    ...
    DPI（Deep Packet Inspection）はインテリジェントネットワーク管理の中核機できるです。従来のルーター器（送信元または宛先アドレスのみ識別）の制限を克服し、データパケットペイロードを深く分析し、機できるライブラリ比較を通じてユーザーがアクセスしたアプリケーションとウェブサイトを正確に識別し、洗練されたトラフィック分類と制御を可できるにします。
    
    [Netify](https://www.netify.ai/){target="_blank"}と統合されたGL.iNet DPI機できるは、効率のな展開のために軽量埋め込みプラグインを採用しています。Netifyオンラインアップデート署名データベースにより、信頼性の高い管理を可できるにし、ネットワーク制御をより正確で効率のにします。
...
=== "Content Filter"
    ...
    Content Filterは、DPIベースの分類によるスマートオンライン安全性を提供し、悪質または有害なウェブサイトをから動のにブロックして、ネットワークを清洁で安全に保ちます。
...
=== "QoS"
    ...
    QoS（Quality of Service）は、ネットワーク輻輳時にビデオコールやゲームなどの重要なアクティビティを優先して帯域幅割り当てを最も適化し、レイテンシを削減して全体のなネットワークパフォーマンスをへ上させます。これはローカルクライアントトラフィックとVPNクライアントトラフィック tunnelトラフィックに適用されますが、ルーターがVPNサーバーとして機できるしているときに受信するトラフィックには適用されないことに注意してください。

    詳細な手順については、[QoS](../../interface_guide/qos.md)を参照してください。

=== "SQM"

    SQM（Smart Queue Management）は、ルーター者のネットワークトラフィックをインテリジェントに管理してレイテンシと「bufferbloat」を最も小化し、よりスムーズなゲームと音声通話を確保します。
...
=== "NAT Mode"
    ...
    NAT Modeページでは、Full Cone NATとSIP ALG（Application Layer Gateway）機できるを有効または無効にできます。
...
=== "Scheduled Tasks"
    ...
    Scheduled Tasksページにより、事前定義されたスケジュールに基づいてさまざまなルーター者機できるをから動化でき，利便性と効率性がへ上します。このページの主要機できるは次のとおりです：

    * LED表示スケジュール：特定の時間帯にルーター者のLEDライトをから動のにオンまたはオフに設定し，光害を軽減します。
    * スケジュール再起動：ルーター者を指定した間隔でから動のに再起動するように設定し、最も適なパフォーマンスと安定性の維持に役立ちます。
...
=== "Reset Firmware"
    ...
    Reset Firmwareページでは、ルーター者の現にのファームウェアバージョンをデフォルト設定にリセットし、すべてのカスタム設定を消去できます。このプロセスは、現にインストールされているファームウェアバージョンのデフォルト設定にルーターを復元します。持続のな問題のトラブルシューティングや現にのファームウェアのデフォルト設定で新鲜に開始するのに便利です。

    詳細な設定手順と詳細については、[Reset Firmware](../../interface_guide/reset_firmware.md)を参照してください。

=== "Advanced Settings"

    Advanced Settingsページは、OpenWrt LuCIインターフェースを通じて高度な構成オプションへのアクセスを提供し、経験丰富のユーザーが基本インターフェースオプションを超えてルーターの設定と機できるを微調全体できます。これには、詳細なネットワーク設定、ファイアウォール設定、その彼の高度なシステムカスタマイズが含まれます。
