...
1. **VPN プロファイルを選択します。**
    ...
    **注意**: 複数のプロファイルを選択した場合、トンネルは優先度順に各プロファイルで接続を試み、接続が確立されるまで順に切り替えます。1 つのトンネル内のすべてのプロファイルで接続に失敗した場合、システムは Tunnel Kill Switch と [All Other Traffic](#all-other-traffic) の状態に基づいて、ローカル WAN へ切り替えるかどうかを判断します。
...
!!! note "Kill Switch とは？"
    ...
    Kill Switch は、VPN 接続のためのセキュリティ機能です。VPN 接続が予期せず切断された場合に、ローカルネットワークのすべてのインターネットアクセスを自動的に遮断し、実際の IP アドレスやオンラインデータの露出を防ぎ、継続的なプライバシーとセキュリティを確保します。公衆ネットワークの利用時、機密データの処理時、実 IP アドレスを隠したい場合など、安全で匿名性の高いインターネットアクセスを維持するのに特に有効です。

    有効にすると、VPN トンネルをバイパスしようとするクライアントトラフィックをすべてブロックし、DNS 設定の問題、予期しない切断、IP 直接指定のリクエストなどによる VPN リークを効果的に防ぎます。

ファームウェア v4.8 以降、GL.iNet ルーターでは各 VPN トンネルごとの Kill Switch と、グローバル VPN 接続の Kill Switch の両方を設定できます。

- 各 VPN トンネルの Kill Switch を設定する方法は、[こちら](#tunnel-options) を参照してください。

- グローバル VPN 接続の Kill Switch（Enhanced Kill Switch）を設定する方法は、[こちら](#all-other-traffic) を参照してください。
