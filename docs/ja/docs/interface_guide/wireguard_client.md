...
このページの内容はファームウェアv4.7以降に基づいています。デバイスで別のファームウェアバージョンを使用している場合は、次の選択メニューから対応するガイドへ切り替えてください。

<div class="gl-link-select" data-label="Firmware version" data-placeholder="Firmware v4.7 and above" markdown="1">

- [ファームウェアv4.6以前](wireguard_client_v4.6.md)
...
GL.iNet ルーターでは、以下の VPN プロバイダー向けに WireGuard の組み込みサポートを提供しています。契約が有効であれば、**WireGuard Client** ページでサービス認証情報を入力するだけで素早く設定できます。

以下の WireGuard サービスプロバイダーから該当するものを選び、手順を確認してください。
...
1. AzireVPN にログインする際の **Username** と **Password** を入力し、**Save and Continue** をクリックします。各サーバー用の設定ファイルが生成されます。
...
[公式サイト](https://hide.me/en/){target="_blank"}

Web 管理パネルで **VPN** -> **WireGuard Client** -> **Hide.me** を開きます。

1. Hide.me にログインする際の **Username** と **Password** を入力し、**Save and Continue** をクリックします。各サーバー用の設定ファイルが生成されます。
...
[公式サイト](https://affiliate.ipvanish.com/aff_c?offer_id=1&aff_id=3073){target="_blank"}

Web 管理パネルで **VPN** -> **WireGuard Client** -> **IPVanish** を開きます。

1. IPVanish にログインする際の **Username** と **Password** を入力し、**Save and Continue** をクリックします。各サーバー用の設定ファイルが生成されます。
...
1. Mullvad の **Account**（16 桁の数字）を入力し、**Save and Continue** をクリックします。各サーバー用の設定ファイルが生成されます。
...
1. [こちら](https://my.nordaccount.com/){target="_blank"} をクリックして、NordVPN の Web アカウントにログインします。
    ...
    Nord Dashboard にログインしたら、左側メニューで **NordVPN** をクリックし、**Access Token** セクションを見つけて **Get Access token** をクリックします。
    ...
    次のページで **Generate new token** をクリックします。
    ...
    ポップアップウィンドウでトークンの有効期限を選択し、**Generate token** をクリックします。
    ...
    するとアクセストークンが表示されます。後で使うためにコピーしてください。

    **注**: アクセストークンは 1 回しか表示されません。この時点で必ずコピーして使用してください。このウィンドウを閉じると、トークンは再表示されません。保存しなかった場合は、新しいトークンを再生成する必要があります。
...
1. PIA にログインする際の **Username** と **Password** を入力し、**Save and Continue** をクリックします。各サーバー用の設定ファイルが生成されます。
...
[PureVPN 15 日間無料トライアル](https://purevpn.com/refer-a-friend-v2?referrer=bEZTci9KcmtQSTFPbXlPak03NWtENWZaL1BHNENlR0JOMzV6RURhWFhSOD0=){target="_blank"}

1. [こちら](https://www.purevpn.com/){target="_blank"} をクリックして、PureVPN のダッシュボードにログインします。
    ...
    ログイン後、左側メニューの **Subscriptions** をクリックし、画面下部に表示される **credentials** を確認します。ユーザー名とパスワードを後で使用するためにコピーしてください。
...
2. ルーターの Web 管理パネルにログインし、**VPN** -> **WireGuard Client** -> **PureVPN** を開きます。

    コピーした **Username** と **Password** を入力し、**Save and Continue** をクリックします。
...
1. Surfshark にログインする際の **Username** と **Password** を入力し、**Save and Continue** をクリックします。各サーバー用の設定ファイルが生成されます。
...
1. Windscribe にログインする際の **Username** と **Password** を入力し、**Save and Continue** をクリックします。各サーバー用の設定ファイルが生成されます。
...
他の WireGuard サービスプロバイダーを契約している場合は、プロバイダーの Web サイトから設定ファイルをダウンロードし、以下の手順で WireGuard Client を設定してください。設定ファイルのダウンロード方法が分からない場合は、[このガイド](../tutorials/how_to_get_configuration_files_from_wireguard_service_providers.md) を参照するか、プロバイダーのサポートへお問い合わせください。
...
## モバイルアプリで WireGuard クライアントを設定する

[モバイルアプリ](https://www.gl-inet.com/app/#download-app-glinet) から WireGuard クライアントを設定することもできます。

モバイルアプリには、AzireVPN、Mullvad VPN、OVPN、StrongVPN、PIA VPN など、一部の WireGuard サービスプロバイダーが統合されています。契約している WireGuard サービスのログイン認証情報を入力するだけで簡単に設定できます。

アプリをインストールし、画面の案内に従って必要な設定を行ってください。
