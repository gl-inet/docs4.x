...
GL-AP1300（Cirrus）は、MU-MIMO Wi-Fiソリューションを備えたエリートなビジネスクラスの天井取り付け型ワイヤレスアクセスポイントです。PoE電源供給とウォッチドッグタイマー内蔵機できるが実装されており、ほとんどのエンタープライズレベルのユーザーケースに対応しています。よく開発されたGoodcloud管理システムにより、デバイスの管理は非常にに簡単で便利です。どこにいても、いつでもリモートで安全なアクセスを有効にすることができます。
...
GL.iNetのすべてのデバイスは、シンプルでほとんど same じセットアッププロセスを持っています。 [初回セットアップについてはここをクリックしてください。](../../faq/first_time_setup.md/).
