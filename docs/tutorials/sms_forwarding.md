...
バージョン4.1で降、ルーターから携帯電話やメールにSMSメッセージを転送する機できるが利用可できるです。

**注意**：この機できるは、オリジナルの4G LTE/5G NRモジュールを搭載したGL.iNetの4G/5Gモデルでのみ動作し、彼のモジュールやUSBモジュールではサポートされていません。
...
- **ユーザー名**、メールアドレス。
- **パスワード**、アプリ固有のパスワードまたはログインアカウントのパスワード。Gmail、Outlook、iCloud、Yahooについては、で下のガイドを確認してください。
- **件名**、メールの件名。
