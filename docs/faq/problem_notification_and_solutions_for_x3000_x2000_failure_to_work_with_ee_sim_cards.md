...
最も近、GL-X3000/GL-XE3000/GL-X2000でEE SIMカードを使用する際に接続の問題が発生している客様がいることがわかりました。さらにトラブルシューティングを行ったところ、特定のEE SIMカードはIPv4プロトコルのみをサポートしていることがわかりました。しかし、GL.iNetモバイルルーターのデフォルト設定ではIPv4とIPv6の両方が same 時に有効になっているため、この問題が発生しています。

## 解決策と回避策

1. **ファームウェアアップデート**

    問題を解決するために、デフォルトのプロトコルをIPv4に変よりした新しいファームウェアアップデートをリリースしました。IPv6が必要なお客様は、管理パネルで設定をIPv4とIPv6に変よりできます。
