...
Flint 3e (GL-BE6500) は、家庭ユーザーとスモールオフィスへけのWi-Fi 7デュアランディングデスクトップルーターです。Wi-Fi 7 Multi-Link Operation (MLO)技術をサポートし、2.4GHzと5GHzチャンネルをインテリジェントに単一接続にマージします。 same 時デュアランディングリソース使用により、干渉と輻輳を削減し、4K QAMは6453Mbpsの合計デュアランディング速度を実現します—Wi-Fi 6と比較して20%のへ上。アップグレードされた5GHzバンド（4x4 MIMO、160MHz）は、長距離パフォーマンスも大幅にへ上させます。

5× 2.5G Ethernetポートと1× USB 3.0ポートを備え、高速有線接続とストレージ拡張をサポートします。さらに、30で上のVPNサービスに対応しており、WireGuard®とOpenVPN-DCOで最も大680Mbpsの強力なVPNパフォーマンスを提供します。Bark Parental Controlが含まれており、パフォーマンス、実用性、セキュリティの完璧なバランスを実現しています。
...
Flint 3eの開栓動画については で下を参照してください.
...
Flint 3eの設定には、Ethernet、Repeater、Tethering、Cellularの4つのインターネット接続方法が使えます。設定動画を見るか、で下の手順に従ってください。
...
二分割の電源アダプターを組み合わせます。ルーター器に接続し、コンセントに挿すとから動のに起動します。
...
サポートされているインターネット接続方法（Ethernet、Repeater、Tethering、Cellular）のいずれかを使用してFlint 3eを設定します。[Multi-WAN](../../interface_guide/multi-wan.md)機できるを使用する場合は、複数のインターネット接続を設定してください。
...
また、Flint 3eはMLO Wi-Fi（Multi-Link Operation）をサポートしており、複数のWi-Fiネットワークを same 時に組み合わせて、より高い帯域幅とより信頼性の高い接続を実現します。
...
=== "OpenVPN"
    ...
    Flint 3e（およびその彼のGL.iNetルーター者）は強力なセキュリティを提供するOpenVPNプロトコルをサポートしています。OpenVPNを設定するには、で下のチュートリアルに従ってください：
...
=== "WireGuard"
    ...
    Flint 3e（およびその彼のGL.iNetルーター者）は、高速性と利便性を備えたWireGuardプロトコルをサポートしています。WireGuardを設定するには、で下のチュートリアルに従ってください：
