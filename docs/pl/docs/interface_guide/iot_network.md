...
> Funkcja IoT Network została wprowadzona w firmware v4.9.

Po lewej stronie webowego panelu administracyjnego przejdź do **NETWORK** -> **IoT Network**.

Ta strona umożliwia utworzenie dedykowanej sieci Wi‑Fi dla urządzeń IoT. Odizolowana od sieci głównej zapewnia lepszą kompatybilność i wyższy poziom bezpieczeństwa.

**Uwaga**: Niektóre modele (np. GL-MT5000, GL-MT2500/GL-MT2500A) nie mają funkcji Wi‑Fi, dlatego ustawienia **IoT Network** nie są dostępne w ich webowym panelu administracyjnym.
...
- **Gateway**
    ...
    **Domyślna brama** sieci IoT to **192.168.10.1**. Jeśli powoduje konflikt z Twoją siecią lokalną, zmień ją na inny adres.
...
- **Block WAN Subnets**
    ...
    Po włączeniu sieć IoT nie może uzyskać dostępu do sieci nadrzędnej ani do jej podsieci.

## Serwer DHCP

Jeśli **IoT Network** jest włączona, jej serwer DHCP również zostanie włączony.

Serwer DHCP automatycznie przypisuje adresy IP i inne parametry komunikacyjne każdemu urządzeniu klienckiemu połączonemu z siecią IoT. Jeśli serwer DHCP jest wyłączony, musisz ręcznie skonfigurować ustawienia sieciowe urządzeń klienckich. Kliknij [tutaj](../tutorials/manually_configure_static_ip.md), aby dowiedzieć się, jak ręcznie skonfigurować statyczny adres IP.
...
- **Gateway**: Urządzenie, które kieruje ruch między siecią IoT a sieciami zewnętrznymi, takimi jak Internet.
