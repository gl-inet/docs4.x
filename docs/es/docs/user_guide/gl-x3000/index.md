# Spitz AX (GL-X3000) Guía del usuario

## Descripción general del producto

Spitz AX (GL-X3000) es una puerta de enlace celular Wi-Fi 6 con doble SIM diseñada para ofrecer conexiones rápidas y fiables, especialmente en zonas remotas y durante viajes por carretera. Ofrece cuatro métodos de acceso a Internet: Cellular (tarjetas SIM), ethernet, repeater y tethering. Es compatible con multi-WAN (failover y balanceo de carga), VPN (OpenVPN y WireGuard), control parental, AdGuard Home, reenvío de puertos, Tailscale y mucho más.
...
| Estado del dispositivo                    | Power                  | Internet               | 2.4GHz                 | 5GHz                   | Cellular               |
| ----------------------------------------- | ---------------------- | ---------------------- | ---------------------- | ---------------------- | ---------------------- |
| Internet conectado (Cellular activa)      | Green On               | Green On               | Green On               | Green On               | Green On               |
| Internet conectado (Cellular inactiva)    | Green On               | Green On               | Green On               | Green On               | Off                    |
| Sin conexión a Internet                   | Green On               | Off                    | Green On               | Green On               | Off                    |
| Actualización de firmware                 | Green On               | Flashing every 0.5s    | Flashing every 0.5s    | Flashing every 0.5s    | Green On               |
| Restablecimiento de red (pulse reset < 3s) | Flashing every 1s    | Green On               | Green On               | Green On               | Green On               |
| Restablecimiento de fábrica (pulse reset 10s) | Flashing every 0.25s | Green On             | Green On               | Green On               | Green On               |

**Consejo**: Cuando esté conectado a Internet, un LED 2.4GHz o 5GHz parpadeando significa que hay dispositivos Wi-Fi conectados y transmitiendo datos activamente.

## Cómo configurar Spitz AX
...
!!! note "Antes de empezar, siga estos pasos (si se conecta mediante cellular)"

    Se requiere al menos una tarjeta nano SIM para conectarse a Internet mediante el método cellular. Cuando tenga lista la(s) tarjeta(s) nano SIM, siga estos pasos:

    1. Active la(s) tarjeta(s) SIM si así lo requiere el operador.
    2. Apague el router.
...
Al confirmar los datos de Wi‑Fi, tenga en cuenta que si cambia la información de Wi‑Fi, deberá volver a conectar su dispositivo al Wi‑Fi del router con las credenciales actualizadas.
...
Configure su Spitz AX usando uno de los métodos de conexión a Internet admitidos: Cellular, Ethernet, Repeater y Tethering. Si desea usar la función [Multi-WAN](../../interface_guide/multi-wan.md), configure más de una conexión a Internet.
...
=== "Cellular"
    ...
    Si no es así, haga clic en la opción **Auto Setup** si aparece.
    ...
    Aprenda a usar la tarjeta física eSIM en un router GL.iNet [aquí](../../tutorials/how_to_use_esim_physical_card_with_glinet_routers.md)
...
Una VPN (red privada virtual) crea tráfico seguro y cifrado entre su dispositivo y el servidor VPN. Proporciona una capa adicional de privacidad y seguridad (cliente VPN) y le permite acceder a una red remota (servidor VPN). Spitz AX (y otros routers GL.iNet) es compatible con OpenVPN y WireGuard. Además, también admite Tor.

=== "OpenVPN"

    Spitz AX (y otros routers GL.iNet) es compatible con el protocolo OpenVPN, que ofrece una seguridad sólida. Para configurar OpenVPN, siga estos tutoriales:
...
=== "WireGuard"
    ...
    Spitz AX (y otros routers GL.iNet) es compatible con el protocolo WireGuard, que ofrece gran velocidad y comodidad. Para configurar WireGuard, siga estos tutoriales:
...
=== "Parental Control"
    ...
    El control parental es un conjunto de ajustes diseñado para ayudarle a administrar y controlar los dispositivos de sus hijos. Incluye limitar su tiempo de pantalla y restringir el acceso a determinados contenidos.
...
=== "Port forwarding"
    ...
    El reenvío de puertos permite que servidores y dispositivos remotos en Internet accedan a dispositivos dentro de una red privada. Para configurar el reenvío de puertos, consulte [Port Forwards](../../interface_guide/port_forwarding.md).
...
=== "Drop-in Gateway"
    ...
    Drop-in Gateway amplía la funcionalidad de su router principal con funciones que quizá no tenga, incluidas AdGuard Home, DNS cifrado y VPN.
...
=== "Network Acceleration"
    ...
    La aceleración por hardware hace referencia al uso de componentes de hardware especializados para realizar tareas específicas de forma más eficiente que las CPU de propósito general.
...
=== "Scheduled Tasks"
    ...
    * Programación del estado del Wi-Fi de 5 GHz: establezca un horario para controlar la banda Wi-Fi de 5 GHz, lo que permite gestionar mejor la disponibilidad de la red y el consumo de energía.
    * Programación del estado del Wi-Fi de 2,4 GHz: establezca un horario para controlar la banda Wi-Fi de 2,4 GHz, lo que permite gestionar mejor la disponibilidad de la red y el consumo de energía.
...
Por la presente, GL TECHNOLOGIES (HONG KONG) LIMITED declara que el tipo de equipo de radio [Router Wi-Fi 6 5G NR, GL-X3000NR / Router Wi-Fi 6 4G LTE, GL-X3000C16] cumple los requisitos esenciales y demás disposiciones pertinentes de la Directiva 2014/53/UE. El texto completo de la declaración UE de conformidad está disponible en la siguiente dirección de internet: [https://www.gl-inet.com/products/certificate](https://www.gl-inet.com/products/certificate){target="_blank"}.