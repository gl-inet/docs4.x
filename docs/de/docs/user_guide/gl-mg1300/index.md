# Mango 2 (GL-MG1300) Benutzerhandbuch
...
Mango 2 (GL-MG1300) ist der erste Dualband-Wi-Fi-5-Mini-Reiserouter von GL.iNet und zeichnet sich durch ein ultraflaches, tragbares Design aus. Er erreicht theoretische Dualband-Geschwindigkeiten von 400 Mbit/s (2,4 GHz) und 866 Mbit/s (5 GHz) bei einer 2×2-MIMO-Konfiguration. OpenVPN und WireGuard sind vorinstalliert; der Router unterstützt mehr als 30 VPN-Dienste, verschlüsselt automatisch den gesamten Netzwerkverkehr und ermöglicht die Fernverwaltung über GoodCloud. Damit vereint er Leistung, Praxistauglichkeit und Sicherheit.
...
- 1 x USB-C-auf-USB-C-Stromkabel
- 1 x Dankeskarte

## So richten Sie Mango 2 ein

Zur Einrichtung von Mango 2 können Sie eine der vier unterstützten Internetverbindungsmethoden verwenden: Ethernet, Repeater, Tethering oder Cellular. Führen Sie die folgenden Schritte aus.

### 1. Einschalten

Schließen Sie das USB-Type-C-Stromkabel an den Stromanschluss des Routers an. Verbinden Sie das andere Ende mit einem 5-V-/2-A-Netzteil (nicht im Lieferumfang enthalten) und stecken Sie dieses in eine Steckdose.

### 2. Gerät verbinden

Verbinden Sie ein Gerät (z. B. Computer, Laptop oder Smartphone) über WLAN oder Ethernet mit dem Router.

- Ethernet

    Verbinden Sie Ihr Gerät über ein Ethernet-Kabel mit dem LAN-Port des Routers.

- Wi-Fi

    Öffnen Sie auf Ihrem Gerät Einstellungen -> WLAN, wählen Sie den WLAN-Netzwerknamen Ihres Routers aus der Liste der verfügbaren Netzwerke und geben Sie das Passwort ein. Den standardmäßigen Netzwerknamen und das Passwort finden Sie auf dem Etikett an der Unterseite des Routers.

### 3. Im Web-Admin-Panel anmelden

Öffnen Sie einen Webbrowser, geben Sie `192.168.8.1` in die Adressleiste ein und melden Sie sich an. Wählen Sie Ihre Sprache, legen Sie das Admin-Passwort fest und klicken Sie anschließend auf **Apply**.

Wenn Sie die WLAN-Daten ändern, müssen Sie Ihr Gerät anschließend mit den aktualisierten Zugangsdaten erneut mit dem WLAN des Routers verbinden.

### 4. Internet einrichten

**Hinweis:** Die folgenden Anweisungen gelten für Benutzer, die den Router über das GL.iNet Web Admin Panel konfigurieren. Wenn Sie stattdessen die GL.iNet-App verwenden möchten, [laden Sie die App herunter](https://www.gl-inet.com/app/){target="_blank"} und folgen Sie den Anweisungen auf dem Bildschirm.

Konfigurieren Sie Mango 2 über eine der unterstützten Internetverbindungsmethoden: Ethernet, Repeater, Tethering oder Cellular. Wenn Sie die Funktion [Multi-WAN](../../interface_guide/multi-wan.md) verwenden möchten, richten Sie mehr als eine Internetverbindung ein.
...
=== "Ethernet"
    ...
    Verbinden Sie den WAN-Port des Mango 2 per Ethernet-Kabel mit einem vorgeschalteten Gerät, zum Beispiel einem Modem.
...
=== "Tethering"
    ...
    1. Verbinden Sie Ihr Mobilgerät, zum Beispiel ein Smartphone oder einen USB-Dongle, per USB-Kabel mit dem USB-Port des Mango 2.
    2. Öffnen Sie auf Ihrem Mobilgerät die Einstellungen und aktivieren Sie **USB Tethering** oder **Personal Hotspot**. Tippen Sie auf einem iPhone auf **Trust This Device**, falls Sie dazu aufgefordert werden.
...
=== "Cellular"
    ...
    Beim Mango 2 können Sie ein USB-C-Modem direkt anschließen oder einen USB-C-auf-USB-A-Adapter verwenden, um ein USB-A-Modem anzuschließen.

    Schließen Sie ein Mobilfunk-USB-Modem an den USB-Port des Mango 2 an. Das ist nützlich, wenn Sie die Internetverbindung eines USB-Modems mit allen verbundenen Geräten teilen möchten.
...
Nachfolgend finden Sie eine Übersicht über die Funktionen im Web-Admin-Panel von Mango 2.

## WLAN

Auf der Seite Wireless können Sie Main Network, Guest Network und IoT Network konfigurieren. Für jeden WLAN-Netzwerktyp lassen sich das 5-GHz- und das 2,4-GHz-Band unabhängig voneinander konfigurieren. Außerdem können Sie für jedes Band grundlegende Einstellungen wie WLAN-SSID, Sicherheitsmodus, Passwort und eine zufällige BSSID festlegen.
...
=== "GL.iNet Account"
    ...
    Das GL.iNet Account ermöglicht Ihnen, Ihre Geräte und Cloud-Dienste zu verbinden und zu verwalten. Sie können nahtlos auf GoodCloud und die glinet App zugreifen und Ihr Netzwerk dadurch sicher und komfortabel jederzeit und von überall verwalten.

    Informationen zur Einrichtung finden Sie unter [GL.iNet Account](../../interface_guide/glinet_account.md).

=== "GoodCloud"

    GL.iNet [GoodCloud](https://www.goodcloud.xyz){target="_blank"} ermöglicht den einfachen Fernzugriff auf GL.iNet-Router und deren Verwaltung.
...
=== "OpenVPN"
    ...
    Mango 2 und andere GL.iNet-Router unterstützen das OpenVPN-Protokoll, das eine hohe Sicherheit bietet. Folgen Sie zur Einrichtung von OpenVPN diesen Anleitungen:
...
=== "WireGuard"
    ...
    Mango 2 und andere GL.iNet-Router unterstützen das WireGuard-Protokoll, das hohe Geschwindigkeiten und eine komfortable Einrichtung bietet. Folgen Sie zur Einrichtung von WireGuard diesen Anleitungen:
...
=== "Subnet"
    ...
    Subnet bündelt die Verwaltung von LAN, Guest Network, IoT Network und benutzerdefinierten VLAN-Netzwerken. So können Sie mehrere Subnetze erstellen und verwalten, um unterschiedliche Geräte- oder Datenverkehrstypen voneinander zu isolieren.

    Informationen zur Einrichtung finden Sie unter [Subnet](../../interface_guide/subnet.md).
...
=== "Ethernet Port"
    ...
    Informationen zur Verwaltung der Ethernet-Ports finden Sie unter [Ethernet Port](../../interface_guide/ethernet_port_v4.10.md).
...
=== "Network Mode"
    ...
    Auf der Seite Network Mode können Sie die Betriebsrolle des Routers an unterschiedliche Netzwerkanforderungen anpassen. Es stehen verschiedene Modi für Szenarien von der WLAN-Abdeckung zu Hause bis zur unternehmensweiten Multi-Link-Vernetzung zur Verfügung. Dabei werden jeweils bestimmte Routerfunktionen aktiviert oder deaktiviert, um die Leistung zu optimieren.
...
=== "Admin Access"
    ...
    Admin Access ermöglicht die Konfiguration verschiedener Sicherheitseinstellungen, die Ihr Netzwerk und den Router vor unbefugtem Zugriff schützen.

    Informationen zur Einrichtung finden Sie unter [Admin Access](../../interface_guide/admin_access.md).
...
=== "Overview"
    ...
    * Flash: Zeigen Sie die Nutzung des Flashspeichers Ihres Routers an, um sicherzustellen, dass genügend Platz für Firmware und Konfigurationsdaten vorhanden ist.
...
=== "Admin Password"
    ...
    Das Admin-Passwort muss folgende Anforderungen erfüllen:

    * Mindestens 10 und höchstens 63 Zeichen.
    * Buchstaben (Groß-/Kleinschreibung beachten), Zahlen und die Symbole `` ! @ # $ % ^ & * ( ) _ + - = , . > < | ? / \ [ ] { } : ; " ' ` ~ `` sind zulässig.
    * Mindestens zwei der folgenden Zeichentypen sind erforderlich: Großbuchstaben, Kleinbuchstaben, Zahlen und Symbole.
