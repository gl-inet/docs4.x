...
GL.iNetルーターの初回セットアップは非常にに似ています。ほとんどの機種にWi-Fiモジュールが搭載されていますが、Wi-Fiモジュールが搭載されていない机型もあります。そのため、で下の案内はWi-Fi モジュールの有無に基づい て2つのケースに分けられます。
...
で下はGL-AXT1800（Slate AX）の例です。

パッケージに same 梱されているで下のものをご用意ください。

- GL-AXT1800
- 電源アダプタ
- イーサネットケーブル

で下のビデオガイドを見るか、で下の手順に従ってください。
...
<small>（このビデオでは、手顺がほとんどのルーター機種で same じため、异なるGL.iNetルーターを使用してセットアップを説明しています。）</small>

1. 電源オン

    TFカードを使用する場合は、ルーターの電源を入れる前に挿入してください。TFカードのホットスワップはサポートされていません。

    電源アダプターの一端をルーターに差し込み、もう一端をコンセントに差し込みます。から動のに電源が入ります。

2. ルーターに接続する

    イーサネットケーブルまたはWi-Fi経よりでルーターに接続できます。
    ...
    * Wi-Fiで接続する
        ...
        Wi-Fi SSIDはルーターの底面ラベルにで下のフォーマットで印刷されています：
...
4. インターネットに接続する
    ...
    * [イーサネットケーブル経よりでインターネットに接続する](../interface_guide/internet_ethernet.md)
    * [既存のWi-Fi経よりでインターネットに接続する](../interface_guide/internet_repeater.md)
    * [USBテザリングでインターネットに接続する](../interface_guide/internet_tethering.md)
    * [USBモデム経よりでインターネットに接続する](../interface_guide/internet_cellular.md)

## Wi-Fi非搭載モデルの場合 {#for-models-that-dont-have-wi-fi}

で下はGL-MT2500A（Brume 2）の例です。

1. 電源オン

    電源アダプターの一端をルーターに差し込み、もう一端をコンセントに差し込みます。ルーターはから動のに電源が入ります。
...
2. ルーターに接続する
    ...
    * 別のルーターのWi-Fiで接続
        ...
        GL-MT2500Aには内蔵Wi-Fiモジュールがないため、別のWi-Fi機できる付きルーターを使用してGL-MT2500Aを初期化できます。
        ...
        * 方法2: 2台目のルーターをデフォルトのルーターモードのままにし、192.168.8.1/24と競合しない別のルーターIPアドレス（例：192.168.10.1）を持っていることを確認し、GL-MT2500AのLANポートを2台目のルーターのWANポートに接続します。
        ...
        !!! 注意
            ...
            ここでいう2台目のルーターとは、GL.iNet GL-AXT1800、TP-LINK、Netgearなどの一般のなルーターです。ISP提供の光ネットワークターミナルまたはデバイスはこのシナリオでは動作しない場合があります。
