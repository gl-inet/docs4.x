...
!!! 注意
    ...
    アップグレード中に電源が入った状態を保ってください。ルーターの電源をオフにしないでください。デバイスはアップグレード後にから動のに再起動します。数秒かかる場合があります。
...
- **プレビュープランを受け入れる**
    ...
    このオプションを有効にすると、最も終バージョンがリリースされる前に新機できるを試すことができ、私たちのフィードバックを提供できます。一度設定するとオンのままになり、オフにするオプションがあります。これらのアップグレードは安定していない場合があることに注意してください。最も新のファームウェア機できるについては、[What's New?](https://www.gl-inet.com/whats-new/){target="_blank"} をご覧ください。

ルーターがインターネットに接続されている場合、利用可できるなファームウェアアップデートプログラムを確認します。
...
**注意**：オンラインアップグレードを実行するとき、「ダウンロード失敗」と表示された場合、**システム** -> **タイムゾーン**に移動し、タイムゾーンエラーを修正します（ブラウザに same 期）。
...
v4.4.12で降、GL.iNetモバイルルーターはウェブ管理パネル経よりでモバイルモジュールアップグレードをサポートします。モバイルネットワークが失敗した場合、モバイルモジュールをアップグレードしてみてください。

### オンラインアップグレード

現にのモバイルモジュールバージョンはここにあります。ルーターがインターネットに接続されている場合、から動のに製造元のサーバーから最も新バージョンを直接確認し、アップデートプロセスを簡素化します。
