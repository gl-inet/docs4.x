# Flint 4 (GL-BE14000) Benutzerhandbuch
...
Flint 4 (GL‑BE14000) definiert die Möglichkeiten eines Heimrouters neu. Er unterstützt Tri-Band-Wi-Fi 7 mit MLO und erreicht Spitzenraten von 688 Mbit/s (2,4 GHz) + 4323 Mbit/s (5 GHz) + 8646 Mbit/s (6 GHz). Für kabelgebundene Verbindungen bietet er eine vollständige Multi-Gigabit-Infrastruktur mit einem 10G-SFP+-WAN/LAN-Port, einem 10GE-WAN/LAN-Port, einem 2,5GE-WAN/LAN-Port, drei 2,5GE-LAN-Ports und vier 1GE-LAN-Ports. Dank leistungsstarker VPN-Unterstützung erreicht er sowohl mit WireGuard® als auch mit OpenVPN DCO einen Durchsatz von bis zu 1,5 Gbit/s. Der integrierte 2,4-Zoll-Touchscreen ermöglicht die Echtzeitüberwachung des Netzwerkstatus und zeigt wichtige Netzwerkkennzahlen direkt am Gerät an.
...
- 1 x Dankeskarte
- 1 x Adapter (abhängig vom Versandland)

Das folgende Video zeigt das Auspacken des Flint 4.
...
Sehen Sie sich das Einrichtungsvideo an oder führen Sie die folgenden Schritte aus.
...
Öffnen Sie einen Webbrowser, geben Sie `192.168.8.1` in die Adressleiste ein und melden Sie sich an. Legen Sie Ihr Admin-Passwort und die Wi-Fi-Einstellungen fest und klicken Sie anschließend auf **Apply**.

### 4. Internet einrichten

Richten Sie Ihren Flint 4 mit einer der unterstützten Methoden für den Internetzugang ein: Ethernet (SFP+), Ethernet (RJ45), Repeater, Tethering und Cellular. Wenn Sie die Funktion [Multi-WAN](../../interface_guide/multi-wan.md) verwenden möchten, richten Sie mehr als eine Internetverbindung ein.
...
=== "Ethernet (SFP+)"
    ...
    Flint 4 verfügt über einen 10G-SFP+-WAN/LAN-Port für Glasfaser-Uplinks, schnelle Switch-Backhauls und leistungsstarke Netzwerkerweiterungen. Dieser Port ist standardmäßig als WAN konfiguriert und kann bei Bedarf auf LAN umgeschaltet werden.

    Das folgende Beispiel zeigt, wie der 10G-SFP+-Port des Flint 4 über einen optischen Transceiver und ein Glasfaserkabel mit dem Glasfaser-Uplink eines Internetanbieters verbunden wird. Weitere Lösungen finden Sie unter [10G-SFP+-Port am Flint 4 anschließen](../../faq/connecting_10g_sfp_plus_port_on_flint4.md).

    1. Stecken Sie einen kompatiblen 10G-SFP+-Transceiver in den SFP+-Port des Flint 4 und verbinden Sie ihn anschließend mit dem Glasfaser-Uplink Ihres Internetanbieters.
    2. Flint 4 versucht, die Netzwerkparameter (IP-Adresse, Gateway und DNS) automatisch über DHCP abzurufen. Wenn Ihr Internetanbieter PPPoE oder eine statische IP-Adresse voraussetzt, passen Sie die WAN-Verbindungseinstellungen im webbasierten Admin Panel entsprechend an.
    3. Sobald die Internetverbindung erfolgreich hergestellt wurde, wird der Ethernet-Bereich auf der Touchscreen-Startseite blau angezeigt (aktiv). Sie können auf dem Touchscreen auf Ethernet tippen oder sich am webbasierten Admin Panel anmelden, um die Verbindungsdetails zu prüfen.

=== "Ethernet (RJ45)"

    ![Ethernet](https://static.gl-inet.com/docs/router/en/4/user_guide/gl-be14000/internet/be14000_ethernet.png){class="glboxshadow"}

    1. Verbinden Sie den WAN-Port des Flint 4 per Ethernet-Kabel mit einem Upstream-Gerät (z. B. ISP-Modem, Netzwerkswitch oder Ethernet-Wandanschluss).
    2. Flint 4 versucht, die Netzwerkparameter (IP-Adresse, Gateway und DNS) automatisch über DHCP abzurufen. Wenn Ihr Internetanbieter PPPoE oder eine statische IP-Adresse voraussetzt, passen Sie die WAN-Verbindungseinstellungen im webbasierten Admin Panel entsprechend an.
...
=== "Repeater"
    ...
    2. Wählen Sie das Wi-Fi-Netzwerk aus, das Flint 4 erweitern soll.
...
=== "Tethering"
    ...
    1. Verbinden Sie ein Mobilgerät (z. B. ein Smartphone) über ein USB-Kabel mit dem USB-Port des Flint 4.
    ...
    3. Wählen Sie auf dem Touchscreen des Flint 4 **Tethering** und tippen Sie auf **Connect**. Der Router stellt daraufhin die Verbindung zu Ihrem Gerät her.
    ...
    **Hinweis**: Wenn die Verbindung fehlschlägt, stellen Sie sicher, dass das Netzteil 12 V/4 A liefert. Bei zu geringer Leistung wird der USB-Port möglicherweise nicht mit Strom versorgt. Wiederholen Sie die obigen Schritte oder melden Sie sich am webbasierten Admin Panel an, um den Status der Tethering-Verbindung zu prüfen.
...
=== "Cellular"
    ...
    1. Schließen Sie ein Mobilfunkmodem oder einen USB-Dongle an den USB-Port des Flint 4 an. So können Sie die Internetverbindung eines USB-Modems mit allen verbundenen Geräten teilen.
...
Nachfolgend finden Sie eine Übersicht über die Funktionen im webbasierten Admin Panel des Flint 4.

## WLAN

Auf der Seite Wireless können Sie verschiedene Wi-Fi-Netzwerke für Ihren Flint 4 konfigurieren, darunter MLO Wi-Fi, Main Network, Guest Network und IoT Network.

Weitere Informationen finden Sie unter [Wireless](../../interface_guide/wireless.md).
...
Ein VPN (Virtual Private Network) erstellt einen sicheren, verschlüsselten Datenverkehr zwischen Ihrem Gerät und dem VPN-Server. Es bietet eine zusätzliche Ebene für Privatsphäre und Sicherheit (VPN-Client) und ermöglicht den Zugriff auf ein entferntes Netzwerk (VPN-Server). Flint 4 unterstützt die Protokolle OpenVPN und WireGuard.

=== "OpenVPN"

    Flint 4 (und andere GL.iNet-Router) unterstützt das OpenVPN-Protokoll, das starke Sicherheit bietet. Zum Einrichten von OpenVPN folgen Sie diesen Anleitungen:
...
=== "WireGuard"
    ...
    Flint 4 (und andere GL.iNet-Router) unterstützt das WireGuard-Protokoll, das hohe Geschwindigkeiten und Komfort bietet. Zum Einrichten von WireGuard folgen Sie diesen Anleitungen:
...
=== "Subnet"
    ...
    Auf der Seite Subnet werden LAN, Guest Network, IoT Network und benutzerdefinierte VLAN Networks zentral verwaltet. Sie können mehrere Subnetze erstellen und verwalten, um unterschiedliche Gerätetypen oder Datenverkehr voneinander zu isolieren.

    Weitere Informationen finden Sie unter [Subnet](../../interface_guide/subnet.md).

=== "Ethernet Port"

    Auf der Seite Ethernet Port können Sie die Portrolle (WAN/LAN) und die VLAN-Segmentierung verwalten sowie Portdetails wie MAC-Adresse und ausgehandelte Geschwindigkeit anzeigen.

    Weitere Informationen finden Sie unter [Ethernet Port](../../interface_guide/ethernet_port_v4.10.md).
...
=== "Network Mode"
    ...
    Der Netzwerkmodus bezeichnet die verschiedenen Betriebsrollen und Funktionen, die ein Router für unterschiedliche Netzwerkanforderungen übernehmen kann. Zu den gängigen Modi gehören Router-, Extender- und Access-Point-Modus.

    Weitere Informationen finden Sie unter [Network Mode](../../interface_guide/network_mode.md).

=== "Drop-in Gateway"

    Drop-in Gateway ist eine flexible Funktion, mit der Sie einen vorhandenen Hauptrouter erweitern können, ohne ihn zu ersetzen oder neu zu konfigurieren. Wenn Sie einen GL.iNet-Router als Drop-in Gateway einrichten, können Sie der bestehenden Netzwerkinfrastruktur erweiterte Funktionen wie AdGuard Home, VPN und verschlüsseltes DNS hinzufügen.

    Informationen zur Einrichtung finden Sie unter den folgenden Links:
...
=== "Network Acceleration"
    ...
    Weitere Informationen finden Sie unter [Network Acceleration](../../interface_guide/network_acceleration.md).
...
=== "ACL"
    ...
    Mit ACL (Access Control List) können Sie Regeln erstellen, um den Netzwerkverkehr anhand von Verbindungsprotokollen, Geräteadressen und Ports zu verwalten. Die Regeln legen fest, ob der Netzwerkzugriff zugelassen oder blockiert wird. Wenn mehrere ACL-Regeln miteinander in Konflikt stehen, wendet das System die Regel mit der höheren Priorität an.

    Weitere Informationen finden Sie unter [ACL](../../interface_guide/acl.md).

=== "Admin Access"

    Mit Admin Access können Sie verschiedene Sicherheitseinstellungen konfigurieren, um Ihr Netzwerk und Ihren Router vor unbefugtem Zugriff zu schützen. Diese Seite umfasst die folgenden Optionen:
...
=== "Bark"
    ...
    Der in Flint 4 integrierte Dienst Bark schützt die digitale Umgebung Ihres Kindes und bietet umfassenden Online-Schutz. Normalerweise ist dafür ein kostenpflichtiges Abonnement erforderlich. Im Rahmen der Partnerschaft mit Bark bietet GL.iNet den Tarif Bark Home jedoch auf ausgewählten Routermodellen einschließlich Flint 4 kostenlos an. Dadurch stehen erweiterte Überwachungs- und Warnfunktionen ohne zusätzliche Kosten zur Verfügung.
...
=== "Admin Password"
    ...
    Auf der Seite Admin Password können Sie das Passwort für die Administrationsoberfläche des Routers verwalten, damit nur autorisierte Benutzer Einstellungen ändern können.
...
=== "Upgrade"
    ...
    * Firmware Online Upgrade: Prüft automatisch auf dem Server des Herstellers, ob die neueste Firmware-Version verfügbar ist. Wenn eine neuere Version online verfügbar ist, können Sie sie installieren.
...
Hiermit erklärt GL TECHNOLOGIES (HONG KONG) LIMITED, dass der Funkanlagentyp [BE14000 Wi-Fi 7 Router, GL-BE14000] den grundlegenden Anforderungen und den übrigen einschlägigen Bestimmungen der Richtlinie 2014/53/EU entspricht. Der vollständige Text der EU-Konformitätserklärung ist unter der folgenden Internetadresse verfügbar: [https://www.gl-inet.com/products/certificate](https://www.gl-inet.com/products/certificate){target="_blank"}.