...
で下の手順は、ルーター管理パネルを通じて OpenVPN クライアントを設定するために書かれています。（GL.iNet モバイルアプリ経よりで OpenVPN クライアントを設定するには、[アプリをダウンロード](https://www.gl-inet.com/app/)して設定してください。）
...
=== "NordVPN"
    ...
    5. 接続したい VPN サーバーの横にある三時アイコンをクリックし、**開始**を選択します。
...
=== "その彼の VPN サービスプロバイダー（例：ExpressVPN、Surfshark）"
...
7. VPN サーバーアドレスの横にある三時アイコンをクリックし、**開始**を選択します。
...
ウェブブラウザでから分の IP アドレスの場所を検索します。接続している VPN サーバーの場所と一致する場合、VPN 接続は成功です。これで設定は完たです。
