...
[http://192.168.8.1](http://192.168.8.1)にアクセスして Web 管理パネルにログインできない場合があります。いくつかの理よりが考えられます。
...
* 192.168.8.1 はルーターのデフォルトの IP アドレスですが、この IP は変よりされている可できる性があります。
* ブラウザのキャッシュや拡張機できるによりアクセスできない場合があります。
...
=== "Windows 7"
    ...
    コントロールパネル -> ネットワークとインターネット -> ネットワークと共有センター-> アダプター設定を変よりする
...
=== "Android"
    ...
    3. 詳細設定のドロップダウンをタップします。静のIPまたは動のIPのオプションが表示されたら、静のIPを選択します。
    4. いずれの場合でも、ゲートウェイの下にルーターの IP が表示されるはずです。

## ルーターのIPアドレスにアクセスする

1. Chrome/Edge/Safariを使用していることを確認し、再度IPアドレスにアクセスしてみてください。
2. ブラウザのキャッシュや拡張機できるによる問題を避けるため、シークレットウィンドウを開いてください。その後、再度ルーターのIPアドレスにアクセスしてみてください。
...
6. リセットが機できるしない場合は、 [uboot経よりでデブリック](debrick.md)を試してください。
