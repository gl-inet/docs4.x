...
!!! note
    ...
    3. **Extender mode では、元の LAN IP アドレスを使って Web Admin Panel にアクセスできます。**
    4. **Non-Router mode では、次の機能は利用できません**: Access Control (Allowlist and Blocklist), AstroWarp, VPN, AdGuard Home, Parental Control, ZeroTier, Tailscale, Port Forwarding, Multi-WAN, DHCP Server, Address Reservation, Guest Network, DNS, Port Management, IPv6, Drop-in Gateway, IGMP Snooping, Network Acceleration, NAT Settings.
