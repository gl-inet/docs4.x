...
すべてのGL.iNetルーターには、物理のなリセット機構（リセットボタンまたはピン穴）が配备されています。ボタンを押すか、ピン穴を刺激すると、同じ効果があります：ネットワーク接続を修復するか、ルーターを工場出荷時のデフォルトにリセットします。

ピン穴付きのモデルの場合は、ピン、伸ばしたクリップ、または類似のツールを使用してアクションを実行してください。

リセットを実行する前に、ルーターが完全に起動していることを確認してください。電源投入直後にリセットボタンを押さないでください。U-Bootフェイルセーフモードがトリガーされる可能性があります。
...
1. 物理のなリセット機構（ボタンまたはピン穴）を使用する
    ...
    **注意：**工場出荷時設定へのリセットが機能しない場合は、[Ubootチュートリアル](debrick.md)に従ってルーターを修復する必要があるかもしれません。
