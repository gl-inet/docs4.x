# Guía de usuario de Slate 7 (GL-BE3600)

## Descripción general del producto

Slate 7 (GL-BE3600) es el primer router de viaje portátil Wi-Fi 7 de doble banda de GL.iNet, construido con tecnologías Wi-Fi 7 de gama alta como Multi-Link Operation y 4K QAM. Ofrece velocidades teóricas de doble banda de 688 Mbps (2,4 GHz) y 2882 Mbps (5 GHz), lo que permite streaming 8K sin interrupciones y experiencias fluidas de juegos móviles. Su pantalla táctil permite comprobar el estado de la red de un vistazo y realizar operaciones básicas de forma intuitiva.

Equipado con 2 puertos Ethernet 2.5G (1 WAN + 1 LAN) y 1 puerto USB 3.0, el router cubre necesidades de conexiones cableadas de alta velocidad y ampliación flexible de almacenamiento. También admite alimentación Type-C PD (5V/3A, 9V/3A, 12V/2.5A) y cuenta con un diseño compacto y ligero para facilitar su transporte. Viene con AdGuard Home preinstalado y es compatible con más de 30 servicios VPN, proporcionando una seguridad sólida para su red. Combinando comodidad móvil con rendimiento de nivel profesional, Slate 7 es una opción ideal para el trabajo remoto y para compartir red en movimiento.
...
- 1 x Adaptador de corriente estadounidense
- 3 x Convertidores (enchufes EU, UK y AU)
...
Vea a continuación el vídeo de unboxing de Slate 7.
...
<small>(En este vídeo mostramos la configuración usando Slate 7 como Repeater. Si necesita configurar Slate 7 mediante otros métodos de conexión a Internet, consulte los pasos que aparecen más abajo).</small>
...
Configure su Slate 7 con uno de los métodos de conexión a Internet compatibles: Ethernet, Repeater, Tethering y Cellular. Si desea utilizar la función [Multi-WAN](../../interface_guide/multi-wan.md), configure más de una conexión a Internet.
...
=== "Ethernet"
    ...
    Conecte el puerto WAN de Slate 7 a un dispositivo aguas arriba, por ejemplo un módem, mediante un cable ethernet.
...
=== "Repeater"
    ...
    1. En la pantalla INTERNET del panel de administración web, localice la sección Repeater y haga clic en **Connect**.
...
=== "Cellular"
    ...
    Conecte un módem USB celular al puerto USB de Slate 7. Esto es útil para compartir Internet desde un módem USB con todos los dispositivos conectados.
...
A continuación se ofrece una visión general de las funciones del panel de administración web de Slate 7.

## Wireless

La página Wireless le permite configurar ajustes para las redes Wi-Fi de 5 GHz y 2,4 GHz, incluyendo activar el Wi-Fi, ajustar la potencia TX, definir el nombre de la red Wi-Fi (SSID), activar el BSSID aleatorio, seleccionar el modo de seguridad Wi-Fi y la contraseña, configurar la visibilidad del SSID y elegir el modo Wi-Fi, el ancho de banda y el canal.

Además, Slate 7 es compatible con MLO Wi-Fi, es decir, Multi-Link Operation, que combina varias redes inalámbricas simultáneamente para lograr mayor ancho de banda y conexiones más fiables.
...
Una VPN (red privada virtual) crea tráfico seguro y cifrado entre su dispositivo y el servidor VPN. Proporciona una capa adicional de privacidad y seguridad cuando se usa como cliente VPN y permite acceder a una red remota cuando se usa como servidor VPN. Slate 7 es compatible con OpenVPN, WireGuard y Tor.

=== "OpenVPN"

    Slate 7, al igual que otros routers GL.iNet, es compatible con el protocolo OpenVPN, que ofrece una gran seguridad. Para configurar OpenVPN, siga estos tutoriales:
...
=== "WireGuard"
    ...
    Slate 7, al igual que otros routers GL.iNet, es compatible con el protocolo WireGuard, que ofrece gran velocidad y comodidad de uso. Para configurar WireGuard, siga estos tutoriales:
...
=== "Scheduled Tasks"
    ...
    * Programación del estado del Wi-Fi: establezca un horario para controlar las bandas Wi-Fi de 5 GHz / 2,4 GHz / MLO, permitiendo gestionar mejor la disponibilidad de la red y el consumo energético.
...
=== "Display Management"
    ...
    La página Display Management le ofrece un conjunto completo de funciones para gestionar la pantalla táctil y sus ajustes relacionados.

    - Function Management: gestione qué funciones se muestran en la pantalla táctil, adaptando la visualización a sus necesidades.
    - Lock Screen: personalice los ajustes de la pantalla de bloqueo. Puede configurar el fondo de pantalla y la activación de la pantalla.
    - Brightness: ajuste el brillo de la pantalla táctil. Use el control deslizante o introduzca un nivel específico (de 1 a 10) para adaptarlo a distintas condiciones de iluminación.
    - Auto Lock: establezca el tiempo de espera para que la pantalla se bloquee automáticamente cuando no haya actividad. El rango va de 1 a 30 minutos.
    - Screen Always On: active o desactive esta opción para decidir si la pantalla táctil permanece encendida continuamente o se apaga tras un periodo de inactividad.
    - Enable Screen Passcode: añada una capa extra de seguridad activando un código para la pantalla táctil.
