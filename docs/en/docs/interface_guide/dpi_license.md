...
1. DPI features are now available only on **GL-MT5000 (Brume 3)**.

2. DPI features will not take effect when the router is in drop-in gateway mode.

---

Log in to the router's web Admin Panel, and go to **FLOW CONTROL** > **DPI License**. 

Start the **30-Day Trial** to enjoy the DPI features, including **Data Statistics**, **Content Filter**, and **QoS**.
