...
AstroMesh は、EasyMesh をベースにした GL.iNet 独自のグローバル Mesh 技術です。単一のローカルネットワーク内でしか動作しない一般的な無線 Mesh システムとは異なり、AstroMesh は場所の制限をなくし、どこからでも自宅ネットワークへ接続できるようにします。
...
- **Slate 7**: Mesh Node として、Main Router の Wi-Fi カバー範囲をローカルで拡張するか、自宅ネットワークをリモート拠点へ拡張します。
...
Slate 7 の Web Admin Panel にログインします。**ASTROMESH** に移動し、**Mesh/Astro Node** をクリックします。

![mesh node](https://static.gl-inet.com/docs/router/en/4/interface_guide/astromesh/sub_node.png){class="glboxshadow"}

### ペアリング

**Wi-Fi Scan** または **Pairing Code** で Mesh Nodes を AstroMesh ネットワークに追加します。
...
??? note "Wi-Fi Scan"
    ...
    1. Mesh Node の電源が入っており、Main ルーターの近くにあること。
    2. Mesh Node で AstroMesh が有効で、**Mesh Node** モードで動作していること。
     ---

    Flint 3 の Web Admin Panel にログインし、**ASTROMESH** に移動します。**Add nodes via Wi-Fi Scan** をクリックします。
...
??? note "Pairing Code"
    ...
    Flint 3 の Web Admin Panel にログインし、**ASTROMESH** に移動します。**Add nodes via Pairing Code** をクリックします。
    ...
    Slate 7 の Web Admin Panel にログインし、**ASTROMESH** に移動します。コピーしたペアリングコードを入力し、**Apply** をクリックします。
    ...
    ***注意**: ペアリングコードには有効期限があります。期限切れになる前に入力してください。*
...
ノードが AstroMesh に正常に追加されると、Main Router の Admin Panel に以下のようなトポロジーが表示されます。
