# WinSCPを使用してGL.iNetルー器のファイルを変更する方法
...
2. ルー器に接続し、WinSCPを実行します。
    ...
    * ホスト名：ルー器のIPを入力します。ルー器のIPを変更していない場合は`192.168.8.1`である必要があります
    * ポート番号：`22`
...
3. ログイン後、ルー器の完全なコントロールが表示されます。
    ...
    例えば、ファイアウォール設定を変更したい場合は、/etc/configに移動し、ファイアウォールファイルを見つけ、右クリックして**Edit**を選択できます。
