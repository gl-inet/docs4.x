...
??? "ソフトウェア設定を確認"
    ...
    **注意:**   
    1. ロックされた基地局は、キャリアとデバイスがサポートする周波数帯と一致する必要があります。一致しない場合、接続に失敗する可能性があります。  
    2. ルーターのセルラーモジュールでは、**International Data Roaming** がデフォルトで有効です。このオプションを無効にすると、SIM カードがインターネットに接続できなくなる場合があります。この機能は有効のままにしてください。
