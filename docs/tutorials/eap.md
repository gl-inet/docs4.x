...
このガイドでは、GL.iNetルーをEAP Wi-Fiネットワークに接続する2つの方法を説明します：管理パネル経よりとLuCI経由です。
...
1. Web管理パネルにアクセスし、**インターネット** -> **ainless**セクションに移動し、**接続**をクリックします。
    ...
    利用可能なネットワークをスキャンします。EAP SSIDを見つけて選択し、直接接続できます。
...
現に、GL.iNet Web管理パネルは少数のEAPタイプのみをサポートしています。大多数の状況ではLuCIページ経由で接続する必要があるかもしれません。
...
- [GL.iNetルー器を高度な設定が必要なEAPネットワークに接続する方法](eap.md)
