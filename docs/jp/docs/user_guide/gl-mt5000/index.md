...
=== "Dynamic DNS"
    ...
    Dynamic DNS（DDNS）は、ドメインに関連付けられたIPアドレスをリアルタイムで自動的に検出およびアップデートします。リモートネットワークにアクセスするために静のIPアドレスが必要なユーザーに最も便利です。
...
=== "Scheduled Tasks"
    ...
    Scheduled Tasksページにより、事前定義されたスケジュールに基づいてさまざまなルーター者機能を自動化でき，利便性と効率性がへ上します。このページの主要機能は次のとおりです：
