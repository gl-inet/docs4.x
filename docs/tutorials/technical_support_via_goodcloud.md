...
- **ファームウェアバージョン4.7.xで上の場合**
    ...
    **クラウドサービス** -> **GoodCloud**に移動し、右上の**始める**をクリックしてクラウドアカウントにログインします。アカウントをお持ちでない場合は、まず登録してください。
    ...
    ログインすると、デバイスはから動のにクラウドアカウントにバインドされます。
...
- **ファームウェアバージョン4.6.xで前の場合**
    ...
    **アプリケーション** -> **GoodCloud**に移動し、**GoodCloud**、**リモートSSH**と**リモートWebアクセス**を有効にし、**利用規約とプライバシーポリシーに same 意**し、**適用**をクリックします。
...
ルー器がファームウェアv4.7で上を実行している場合は、ルー器のWeb管理パネルでアカウントを登録して直接ログインできます。ログインすると、現にのデバイスはから動のにクラウドアカウントにバインドされます。

ルー器がファームウェアv4.6で前を実行している場合は、で下の手順に従ってアカウントにログインし、デバイスをバインドします。
