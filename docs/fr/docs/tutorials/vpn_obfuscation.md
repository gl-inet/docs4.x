...
- Elle masque les caractéristiques du VPN afin d'empêcher la détection par les fournisseurs d'accès Internet (FAI), les pare-feu ou l'inspection approfondie des paquets (DPI).
...
AmneziaWG est un protocole VPN construit sur WireGuard, avec une obfuscation du trafic intégrée. Il conserve les principaux avantages de WireGuard, tels que la vitesse élevée, la conception légère et la faible latence, tout en ajoutant un module d'obfuscation dédié. Ce module masque efficacement les modèles de trafic VPN, permettant aux utilisateurs particuliers comme aux entreprises de protéger leur vie privée en ligne, de contourner les restrictions régionales et d'éviter les interruptions de connexion causées par des contrôles réseau stricts.
...
Actuellement, plusieurs routeurs GL.iNet (par ex. **Brume 3**, **Flint 3**, **Flint 2** et **Beryl AX**) prennent en charge le protocole AmneziaWG dans certaines versions de firmware. La prise en charge officielle complète sera disponible dans le firmware ver.4.9 et sera progressivement déployée sur davantage de modèles.
...
5. Exporter le fichier de configuration.
...
3. Téléverser le fichier de configuration.
...
Il n'est pas nécessaire de configurer votre propre serveur. Vous pouvez établir une connexion VPN obfusquée directement à l'aide des serveurs officiels AmneziaVPN (abonnement Amnezia Premium requis).
