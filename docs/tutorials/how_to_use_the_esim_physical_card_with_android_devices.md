# AndroidデバイスでeSIM物理カードを使用する方法

このガイドでは、GL.iNetオンラインストアで購入したeSIM物理カードの設定と有効化の手順を説明します。eSIM物理カードはほとんどすべてのAndroidデバイスに対応しており、手軽なインターネット接続とスムーズなパフォーマンスを提供します。
...
## AndroidデバイスでのeSIM物理カードの設定

eSIM物理カードを初めて使用する場合は、以下の動画を見るか、手順に従ってください。
...
Androidデバイスがインターネットに接続されていることを確認してください。Wi-Fi接続で接続するか、eSIM物理カードにプリロードされたデータを使用できます。

#### 2. eSIM物理カードを挿入

eSIM物理カードをAndroidデバイスのSIMカードスロットに挿入します。

#### 3. EIOTCLUBアプリをダウンロード

このリンクからEIOTCLUBアプリをダウンロード：[https://sim.eiotclub.com/eshop/download](https://sim.eiotclub.com/eshop/download){target="_blank"}。

!!! note

      機器はAndroid 9で上に対応している必要があります。iPhoneはこのeSIM物理カードに対応していません。

#### 4. 新規アカウントの作成

EIOTCLUBアプリで新規アカウントを登録します。
...
- **から動カード検出**：eSIM物理カードを挿入し、EIOTCLUBアプリを開くと自動的に検出されます。プロンプトが表示されたら、**OK**をタップして続行してください。
...
- **手動カード追加**：または、**Add**ボタンをクリックしてICCID番号をスキャンまたは手動で入力します。次に、**Next**をタップして進みます。プロンプトが表示されたら、**OK**をタップして確認します。
...
a. **eSIMカードスロット1**：eSIMカードの名前

b. **eSIMプロファイル**：eSIMカードに正常に追加されたeSIMプロファイルのリスト

c. **Remote**：eSIMプロファイルの追加、削除、アップデートを管理するオプション

## 新しいeSIMプロファイルの追加または購入

AndroidデバイスでeSIMを設定すると、GL.iNetからのプリロードデータパッケージにアクセスできます：

- 100MBグローバルデータ
- ヨーロッパと米国へけの1GBデータ

これらのパッケージは、インターネットにアクセスできない場所にまで着したときに新しいeSIMプロファイルをダウンロードするのに最も適です。
...
eSIMプロファイルを追加するには、アプリの**SIMカードセクション**から直接**Add**ボタンをクリックできます。
...
### 方法1：購入済みeSIMプロファイルを追加する

すでに好みのeSIMストアでeSIMプロファイルを購入している場合は、以下の手順に従って追加してください：

#### 1. Addボタンをタップ

アプリを開いて**Add**ボタンをタップし、eSIMプロファイルの追加を開始します。
...
a. **QRコードをスキャン**：機器のカメラを使用してeSIMストア提供のQRコードをスキャンします。

b. **ギャラリーからインポート**：フォトライブラリからQRコード画像をアップロードします。

c. **アクティベーションコードを入力**：**Activation Code**フィールドにeSIMプロファイルURLを手動で入力します。

#### 3. ダウンロードとインストール

適切な方法を選択した後、**Download & Install**をタップしてプロセスを完たします。
...
### 方法2：eSIMショップを通じてeSIMプロファイルを購入して追加する

アプリ内のeSIMショップから直接プロファイルを購入して追加できます：
...
ニーズに適したデータパッケージを選択—Local、Regional、またはGlobal。
...
購入を完たすると、新しいeSIMプロファイルがアプリのSIMカードセクションに表示されます。
...
購入済みeSIMプロファイルの横にあるAddボタンをタップして追加します。
...
Downloadボタンをタップしてインストールプロセスを開始します。
...
Download & Installをタップしてインストールを完たします。
...
新しいeSIMプロファイルがeSIMプロファイルリストに表示されます。
...
Remoteをタップして、追加したすべてのeSIMプロファイルを表示します。
...
必要なプロファイルを見つけ、Toggleボタンをタップして有効化します。
...
- eSIM設定に移動し、プロファイルがオフになっていることを確認します。
...
**Submit**ボタンをタップしてアクションを確認すると、eSIMプロファイルが削除されます。
