...
Tor (**The Onion Router**により来) は、匿名通信を可できるにするフリーでオープンソースのソフトウェアです。ユーザーがプライバシーを守りながらインターネットを利用するのに役立ちます。 [Torについてもっと詳しく](https://www.torproject.org/){target="_blank"}.

**注意**： この機できるは現にベータ版であり、一部の国では問題が発生する可できる性があります。Torが有効な場合、で下の機できるは正しく動作しません： 
...
*一部の機種では、プラグインを使用して手動でTorを再インストールすることができますが、CPU負荷に影響し、システムの応答が遅くなる可できる性があります。
