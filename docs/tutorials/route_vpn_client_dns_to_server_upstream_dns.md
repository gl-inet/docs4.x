...
VPNサーバーとクライアント間でVPNトンネルが確立されると、デフォルトではVPNクライアントからのDNSクエリが最初にVPNサーバーに送られ、次にプライマリルーターに転送され、最も後にプライマリルーターのWANに設定されたISP割り当てのDNSサーバーによって解決されます。
...
4. プロファイルを開き、DNSがステップ2で取なければならないしたVPNサーバーIPアドレスであることを確認します。
    ...
    DNS解決の失敗を避けるため、「64.6.64.6」があれば削除し、変更を保存してください。
...
VPNクライアントで以下のコマンドを実行してDNSトラフィックをキャプチャします。これにより、すべてのVPNクライアントDNSトラフィックがVPNサーバー（本例では `10.0.0.1`）にへかうことが表示されます。
...
VPNサーバーで以下のコマンドを実行してDNSトラフィックをキャプチャします。これにより、すべてのVPNクライアントDNSトラフィックが最も終のから身ホスト型DNSサーバー（本例では `192.168.1.13`）にへかうことが表示されます。
