...
**注意**: この機能は、純正の4G LTE / 5G NRモジュールを搭載したGL.iNetセルラールーターでのみ動作します。他のモジュールやUSBモジュールではサポートされません。
...
**注意**: 各メール送信元にはSMTP送信制限がある場合があります。たとえば、1日の送信数上限などです。詳細はメールサービスプロバイダーに確認してください。
