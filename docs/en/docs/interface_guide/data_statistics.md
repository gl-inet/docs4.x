...
3. Data Statistics only tracks traffic usage for devices listed on the Client page. If a device connects to the router through a tunnel interface (e.g., VPN Client, Tailscale, or AstroWarp), any traffic forwarded by the router from that device is not included in the statistics.
