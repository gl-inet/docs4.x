...
!!! Note
    ...
    デバイスのバージョンに応じて、正しいアンテナコネクタを選択してください。互換性のないコネクタを使用すると、アンテナの故障や信号性能の異常が発生する可能性があります。各バージョンのアンテナインターフェースは次のとおりです。

    - X300B‑GPS: SMB オスコネクタ x 5  
    - X300B‑RS485: RP‑SMA メスコネクタ x 2、SMA メスコネクタ x 1   
    - X300B‑BLE: RP‑SMA メスコネクタ x 3
   
    アンテナは対応する嵌合タイプのコネクタを使用する必要があります。デバイスポートがオスの場合はメスコネクタのアンテナを使用し、デバイスポートがメスの場合はオスコネクタのアンテナを使用してください。
