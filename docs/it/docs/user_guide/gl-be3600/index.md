# Guida utente di Slate 7 (GL-BE3600)
...
- 1 x Alimentatore US
- 3 x Convertitori (spine EU, UK e AU)
...
Guarda qui sotto il video di unboxing di Slate 7.
...
<small>(In questo video mostriamo la configurazione di Slate 7 in modalita Repeater. Se devi configurare Slate 7 con altri metodi di connessione Internet, fai riferimento ai passaggi sotto.)</small>
...
??? "Accensione"
    ...
    Quando il router viene acceso, il logo GL.iNet viene visualizzato sullo schermo, seguito dalla barra di avanzamento dell'avvio.

    ![booting](https://static.gl-inet.com/docs/router/en/4/user_guide/gl-be3600/touchscreen/booting.png){class="glboxshadow"}
    
    Quando la barra di avanzamento e completamente caricata, l'avvio del dispositivo e terminato.

??? "Rete"

    La schermata iniziale mostra quattro icone che rappresentano quattro tipi di connessione di rete: Ethernet, Repeater, Tethering e Cellular.
    ...
    | Colore | Stato |
    | :-- | :-- |
    | Verde | Attivo / Connesso a Internet |
    | Giallo | Connessione in corso / Errore di rete |
    | Bianco | Connessione inattiva |

    Tocca una qualsiasi di queste icone per visualizzare lo stato della rete o informazioni dettagliate di configurazione.

??? "Funzioni"

    Puoi accedere alle funzioni scorrendo verso sinistra e verso destra.

    Scorrendo da destra a sinistra, vengono visualizzati in sequenza gli elementi seguenti; alcuni richiedono una configurazione preliminare nel pannello di amministrazione. Scorrendo da sinistra a destra, l'ordine di visualizzazione si inverte.

    - Connessione di rete

        ![network connection](https://static.gl-inet.com/docs/router/en/4/user_guide/gl-be3600/touchscreen/network.png){class="glboxshadow"}

    - Dettagli Wi-Fi tri-band (inclusi SSID, password, codice QR e pulsante di attivazione)
    ...
    - Statistiche del traffico (si riferiscono alla velocita media di tutto il traffico che passa attraverso il router. La velocita viene calcolata ogni 3 secondi.)
...
??? "Sistema"
    ...
    Scorri dall'alto verso il basso per accedere alle impostazioni di sistema: Reboot e Lock Screen.
    ...
    - Reboot: toccando "Reboot" verra visualizzato il prompt "Slide To Reboot" (verifica in due passaggi), quindi il router avviera il processo di riavvio.
    ...
    - Lock Screen: lo schermo si spegnera toccando l'opzione "Lock Screen". Tocca lo schermo per riattivarlo: verra visualizzata la pagina funzione in cui era rimasto. Toccando di nuovo verra visualizzato il prompt "Slide To Unlock".
...
- Wi-Fi
    ...
    Sul dispositivo, individua il nome della rete Wi-Fi del router nell'elenco delle reti disponibili e inserisci la password per collegarti. Il nome di rete e la password predefiniti sono stampati sull'etichetta del router.

### 4. Accedi al pannello di amministrazione web

Apri un browser web, inserisci `192.168.8.1` nella barra degli indirizzi ed effettua l'accesso. Scegli una lingua e imposta la password di amministrazione, quindi fai clic su **Apply**.

### 5. Configurazione della connessione Internet
...
Configura Slate 7 usando uno dei metodi di connessione Internet supportati: Ethernet, Repeater, Tethering e Cellular. Se desideri usare la funzione [Multi-WAN](../../interface_guide/multi-wan.md), configura piu di una connessione Internet.
...
=== "Ethernet"
    ...
    Collega la porta WAN di Slate 7 a un dispositivo upstream, ad esempio un modem, tramite un cavo Ethernet.
...
=== "Repeater"
    ...
    1. Nella schermata INTERNET del pannello di amministrazione web, individua la sezione Repeater e fai clic su **Connect**.
...
=== "Cellular"
    ...
    Inserisci un modem USB cellulare nella porta USB di Slate 7. Questo metodo e utile per condividere la connessione Internet di un modem USB con tutti i dispositivi collegati.
...
Di seguito trovi una panoramica delle funzioni presenti nel pannello di amministrazione web di Slate 7.
...
Una VPN (rete privata virtuale) crea traffico sicuro e crittografato tra il dispositivo e il server VPN. Offre un ulteriore livello di privacy e sicurezza come client VPN e consente di accedere a una rete remota come server VPN. Slate 7 supporta OpenVPN, WireGuard e Tor.

=== "OpenVPN"

    Slate 7, come gli altri router GL.iNet, supporta il protocollo OpenVPN, che offre un elevato livello di sicurezza. Per configurare OpenVPN, segui questi tutorial:
...
=== "WireGuard"
    ...
    Slate 7, come gli altri router GL.iNet, supporta il protocollo WireGuard, che offre ottima velocita e praticita. Per configurare WireGuard, segui questi tutorial:
...
=== "Plug-ins"
    ...
    Un plugin e un componente software che aggiunge funzioni o funzionalita specifiche a un programma informatico esistente, consentendo personalizzazione e ampliamento delle sue capacita.
...
=== "Ethernet Port"
    ...
    La pagina Ethernet Port consente di configurare le porte WAN e LAN, impostare l'interfaccia WAN/LAN su Ethernet, specificare modalita MAC e indirizzo MAC per l'interfaccia WAN e visualizzare la velocita negoziata della porta di rete.
...
=== "Display Management"
    ...
    - Function Management: gestisci le funzioni mostrate sul touchscreen, adattando la visualizzazione alle tue esigenze.
    - Lock Screen: personalizza le impostazioni della schermata di blocco. Puoi impostare lo sfondo e il risveglio del display.
    - Brightness: regola la luminosita del touchscreen. Usa il cursore oppure inserisci un livello specifico, da 1 a 10, per adattarlo a diverse condizioni di luce.
    - Auto Lock: imposta il ritardo di tempo per il blocco automatico dello schermo in assenza di attivita. L'intervallo va da 1 a 30 minuti.
    - Screen Always On: attiva o disattiva questa opzione per decidere se il touchscreen deve restare sempre acceso oppure spegnersi dopo un periodo di inattivita.
    - Enable Screen Passcode: aggiungi un ulteriore livello di sicurezza abilitando un codice per il touchscreen.
