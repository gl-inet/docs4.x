...
**Activation Link** がメールアドレスに送信されます。受信トレイを開いて ExpressVPN アカウントを有効化します。
...
メール内の **Activate Subscription** ボタンをクリックします。
...
アカウントパスワードの作成ページに移動します。続いて **Set Password** をクリックします。
...
アカウントが正常に有効化されました。Fortify ルーターの Web Admin Panel に戻り、アカウントでログインしてください。
...
Fortify の Web Admin Panel で **VPN** -> **VPN Client Profile** に移動し、**Log in to ExpressVPN** をクリックします。
...
ルーターで ExpressVPN へのアクセスを許可するため、**Yes** をクリックします。
...
ログインに成功しました。このウィンドウを閉じてもかまいません。
...
Fortify の Web Admin Panel で **VPN** -> **VPN Client Profile** に移動します。このルーターで ExpressVPN にログイン済みです。**Go to ExpressVPN Dashboard** をクリックします。
...
これで、必要に応じて VPN トンネルを追加し、VPN ポリシーを設定できます。
