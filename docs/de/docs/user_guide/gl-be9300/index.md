# Flint 3 (GL-BE9300) Benutzerhandbuch
...
Flint 3 (GL-BE9300) ist ein Tri-Band-Wi-Fi-7-Desktop-Router für Heimanwender, kleine Büros und anspruchsvolle Einsatzszenarien. Er unterstützt Wi-Fi 7 Multi-Link Operation (MLO) und kombiniert seine 2.4GHz-, 5GHz- und 6GHz-Bänder intelligent zu einer Verbindung, um Interferenzen und Überlastung zu reduzieren. In Verbindung mit 4K QAM erreicht er theoretische Tri-Band-Geschwindigkeiten von 688 Mbit/s (2.4GHz), 2882 Mbit/s (5GHz) und 5765 Mbit/s (6GHz). MLO oder 6GHz-Konnektivität liefern Spitzenleistung, während das 2.4GHz-Band für IoT-Geräte reserviert werden kann.
...
Sehen Sie sich unten das Unboxing-Video von Flint 3 an.
...
- Wi-Fi
  ...
  Öffnen Sie auf Ihrem Gerät Einstellungen -> WLAN, suchen Sie in der Liste der verfügbaren Netzwerke den Wi-Fi-Netzwerknamen Ihres Routers und geben Sie das Passwort ein, um dem Netzwerk beizutreten. Den Standard-Netzwerknamen und das Standard-Passwort finden Sie auf dem Etikett des Routers.
...
Richten Sie Flint 3 mit einer der unterstützten Internetverbindungsmethoden ein: Ethernet, Repeater, Tethering und Cellular. Wenn Sie die Funktion [Multi-WAN](../../interface_guide/multi-wan.md) verwenden möchten, richten Sie bitte mehr als eine Internetverbindung ein.
...
Nachfolgend finden Sie eine Übersicht über die Funktionen des Flint-3-Web-Admin-Panels.

## WLAN

Auf der Seite Wireless können Sie Einstellungen für die 6GHz-, 5GHz- und 2.4GHz-Wi-Fi-Netzwerke konfigurieren, darunter das Aktivieren von Wi-Fi, das Festlegen der TX-Leistung, das Definieren des Wi-Fi-Namens (SSID), das Aktivieren randomisierter BSSID, die Auswahl des Wi-Fi-Sicherheitsmodus und Passworts sowie die Konfiguration von SSID-Sichtbarkeit, Wi-Fi-Modus, Bandbreite und Kanal.

Zusätzlich unterstützt Flint 3 MLO-Wi-Fi, also Multi-Link Operation, wobei mehrere drahtlose Netzwerke gleichzeitig kombiniert werden, um höhere Bandbreite und zuverlässigere Verbindungen zu erreichen.
...
Ein VPN (virtuelles privates Netzwerk) erstellt einen sicheren, verschlüsselten Datenverkehr zwischen Ihrem Gerät und dem VPN-Server. Es bietet eine zusätzliche Ebene für Datenschutz und Sicherheit (VPN-Client) und ermöglicht Ihnen den Zugriff auf ein entferntes Netzwerk (VPN-Server). Flint 3 unterstützt OpenVPN, WireGuard und Tor.

=== "OpenVPN"

    Flint 3 und andere GL.iNet-Router unterstützen das OpenVPN-Protokoll, das hohe Sicherheit bietet. Folgen Sie zum Einrichten von OpenVPN diesen Anleitungen:
...
=== "WireGuard"
    ...
    Flint 3 und andere GL.iNet-Router unterstützen das WireGuard-Protokoll, das hohe Geschwindigkeiten und eine komfortable Nutzung bietet. Folgen Sie zum Einrichten von WireGuard diesen Anleitungen:
...
=== "Network Mode"
    ...
    Network Mode bezeichnet die Konfigurationseinstellungen, die festlegen, wie ein Gerät sich mit einem Netzwerk verbindet und mit anderen Geräten kommuniziert.

    Zum Einrichten von Network Mode lesen Sie bitte [Network Mode](../../interface_guide/network_mode.md).
...
=== "IGMP Snooping"
    ...
    IGMP Snooping ist eine Netzwerkoptimierungstechnik, die in Ethernet-Switches verwendet wird, um Multicast-Verkehr zu verwalten und zu steuern.
...
=== "NAT-Einstellungen"
    ...
    Auf der Seite NAT Settings können Sie Full Cone NAT und die SIP-ALG-Funktion (Application Layer Gateway) aktivieren oder deaktivieren.
...
=== "Overview"
    ...
    * Memory Usage: Prüfen Sie, wie viel Arbeitsspeicher Ihres Routers derzeit genutzt wird, um Ressourcen besser verwalten zu können.
    * LED Control: Schalten Sie die LED-Leuchten des Routers ein oder aus, um die optischen Anzeigen des Geräts anzupassen.
    * Flash Usage: Zeigen Sie die Auslastung des Flash-Speichers des Routers an, damit ausreichend Platz für Firmware- und Konfigurationsdaten verfügbar bleibt.
    * Device Info: Greifen Sie auf detaillierte Systeminformationen Ihres Routers zu, darunter Betriebszeit, Hostname, Modell, Architektur, OpenWrt-Version, Kernel-Version, Geräte-ID, Geräte-MAC und Geräte-S/N.
...
=== "Upgrade"
    ...
    Die Seite Upgrade dient dazu, die Firmware Ihres Routers auf die neueste Version zu aktualisieren, um Leistung, Sicherheit und Funktionsumfang zu verbessern. Diese Seite bietet zwei Upgrade-Optionen:

    * Firmware Online Upgrade: Prüfen Sie automatisch auf die neueste Firmware-Version und installieren Sie diese direkt vom Server des Herstellers, was den Update-Prozess vereinfacht.
    * Firmware Local Upgrade: Laden Sie manuell eine Firmware-Datei von Ihrem Computer hoch, um den Router zu aktualisieren, und behalten Sie so die Kontrolle über Version und Zeitpunkt des Upgrades.
...
=== "Scheduled Tasks"
    ...
    * Wi-Fi Status Schedule: Legen Sie einen Zeitplan zur Steuerung des 6GHz-/5GHz-/2.4GHz-/MLO-Wi-Fi-Bands fest, um Netzwerkverfügbarkeit und Stromverbrauch besser zu verwalten.
...
Hiermit erklärt GL TECHNOLOGIES (HONG KONG) LIMITED, dass der Funkanlagentyp [BE9300 Wi-Fi 7-Router, GL-BE9300] den grundlegenden Anforderungen und anderen relevanten Bestimmungen der Richtlinie 2014/53/EU entspricht. Der vollständige Text der EU-Konformitätserklärung ist unter folgender Internetadresse verfügbar: [https://www.gl-inet.com/products/certificate](https://www.gl-inet.com/products/certificate){target="_blank"}.