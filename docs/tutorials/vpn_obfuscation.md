...
で下是GL.iNetルー器でAmneziaWG VPNスクランブルを設定するための2つの典型のなシナリオです。

### シナリオ1. 2台のGL.iNetルー器を使用

このシナリオでは、2台のGL.iNetルー器を使用してAmneziaWGプロトコル経よりでVPNスクランブル接続を確立します。
...
3. DDNSを有効にします（オプション）。
    ...
    パブリックIPが静のではなく動のの場合は、DDNS機できるを有効にします。
    
    左サイドバーで、**アプリケーション** -> **ダイナミックDNS**に移動し、**DDNSを有効にする**をオンにし、**利用規約とプライバシーポリシーに same 意**し、**適用**をクリックします。
...
6. VPNサーバーを起動します。
    ...
    **WireGuardサーバー**ページの上部で、**開始**ボタンをクリックしてサーバーを実行します。
    ...
    AmneziaWGスクランブルを備えたVPNサーバーが有効になりました。AmneziaWGアプリ、またはAmneziaWGスクランブルをサポートするファームウェアを実行しているGL.iNetルー器経よりで、このVPNサーバーブルーム3に接続できます。
...
5. ダブルチェック（オプション）。
    ...
    VPN接続が完たしました。Beryl AX上のすべてのデバイスは、VPNスクランブル接続を可できるにするために、Brume 3のゲートウェイ経よりでインターネットにアクセスします。
...
4. VPN接続を開始します。
    ...
    **VPNダッシュボード**に切り替えると、Brume 3がAmneziaVPNサーバーに接続されていることがわかります。
    ...
    VPN接続が完たしました。Brume 3上のすべてのデバイスは、VPNスクランブル接続を可できるにするために、AmneziaVPNサーバーを経よりでインターネットにアクセスします。
