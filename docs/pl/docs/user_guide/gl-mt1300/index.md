# Przewodnik użytkownika Beryl (GL-MT1300)

## Opis produktu

Beryl (GL-MT1300) to wysokowydajny, kieszonkowy router, który łączy mocne podzespoły, zaawansowane protokoły cyberbezpieczeństwa oraz unikalne, nowoczesne wzornictwo. Jako bardziej zaawansowana wersja naszego bestsellera, Slate (GL-AR750S), Beryl wyznacza nową generację routerów podróżnych.
...
Obejrzyj [film z rozpakowania Beryl](../../video_library/unboxing_first_set_up.md#gl-mt1300-beryl).

## Specyfikacja

[Specyfikacja GL-MT1300](https://www.gl-inet.com/products/gl-mt1300/#specs){target="_blank"}
...
Zaloguj się do webowego panelu administracyjnego routera, a następnie przejdź do **INTERNET** w menu po lewej stronie.
...
Skonfiguruj router jako repeater, aby rozszerzyć zasięg istniejącej sieci Wi-Fi. W tym trybie urządzenie odbiera i retransmituje sygnał bezprzewodowy w swoim zasięgu, zwiększając obszar pokrycia. Ta metoda jest przydatna, gdy jeden router nie jest w stanie objąć całego obszaru użytkowania.
...
Multi-WAN to funkcja sieciowa, która umożliwia jednoczesne skonfigurowanie routera z wieloma połączeniami internetowymi (np. Ethernet i Repeater). Jeśli połączenie internetowe o najwyższym priorytecie przestanie działać, router automatycznie przełączy się na inne połączenie. Jest to nazywane przełączeniem awaryjnym i zapewnia płynny, nieprzerwany dostęp do internetu.

Przejdź do [Multi-WAN](../../interface_guide/multi-wan.md), aby ustawić priorytet każdego połączenia internetowego.

Możesz także przełączyć tryb Multi-WAN z przełączenia awaryjnego na równoważenie obciążenia, co pozwala jednocześnie korzystać z wielu interfejsów sieciowych w celu zwiększenia całkowitej przepustowości routera.
...
Routery GL.iNet mają fabrycznie zainstalowane funkcje klienta VPN i serwera VPN dla OpenVPN oraz WireGuard.
...
Szczegółowe instrukcje konfiguracji znajdziesz w poniższych odnośnikach:
...
Routery GL.iNet oferują wiele funkcji zapory, które pomagają zapewnić bezpieczne połączenie i pełną kontrolę użytkownika. Umożliwiają konfigurowanie reguł zapory, w tym Port Forwarding, Open Ports i DMZ.
