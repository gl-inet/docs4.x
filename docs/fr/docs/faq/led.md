# États des voyants LED

Ce document décrit les états des voyants LED et leur signification pour différents modèles de routeurs GL.iNet. 

## Routeur de voyage

### GL-MT3600BE

![gl-mt3600be interface](https://static.gl-inet.com/docs/router/en/4/faq/led/mt3600be_interface.png){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-MT3000

![gl-mt3000 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/mt3000_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-AXT1800

![gl-axt1800 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/gl-axt1800_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-SFT1200

![gl-sft1200 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/sft1200_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-MT1300

![gl-mt1300 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/mt1300_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-A1300

![gl-a1300 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/gl-a1300_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

## Routeur cellulaire

### GL-XE3000

![gl-xe3000 led](https://static.gl-inet.com/docs/router/en/4/faq/led/xe3000led.jpg){class="glboxshadow gl-70-desktop"}

* LED d'alimentation
	* Vert : sous tension
	* Éteinte : hors tension

* LED de batterie (pendant la charge)
	* Rouge clignotant : niveau de batterie 0 %-20 %
	* Violet clignotant : niveau de batterie 21 %-50 %
	* Vert clignotant : niveau de batterie 51 %-99 %
	* Vert fixe : niveau de batterie 100 %

* LED de batterie (hors charge)

	Lorsqu'il n'est pas en charge, le voyant de batterie s'allume pendant 5 secondes après avoir appuyé sur le bouton d'alimentation.

	* Rouge : niveau de batterie 0 %-20 %
	* Violet : niveau de batterie 21 %-50 %
	* Vert : niveau de batterie 51 %-100 %
	
* LED Internet
	* Allumée : Internet est connecté
	* Éteinte : Internet est déconnecté

* LED Wi-Fi 2.4G
	* Allumée : le Wi-Fi 2.4G est prêt
	* Clignotante : le Wi-Fi 2.4G communique

* LED Wi-Fi 5G
	* Allumée : le Wi-Fi 5G est prêt
	* Clignotante : le Wi-Fi 5G communique

* LED cellulaire
	* Connexion au réseau mobile établie

### GL-X3000

![gl-x3000 led](https://static.gl-inet.com/docs/router/en/4/faq/led/gl-x3000_led.png){class="glboxshadow"}

* LED d'alimentation
	* Vert : sous tension
	* Éteinte : hors tension

* LED Internet
	* Allumée : Internet est connecté
	* Éteinte : Internet est déconnecté

* LED Wi-Fi 2.4G
	* Allumée : le Wi-Fi 2.4G est prêt
	* Clignotante : le Wi-Fi 2.4G communique

* LED Wi-Fi 5G
	* Allumée : le Wi-Fi 5G est prêt
	* Clignotante : le Wi-Fi 5G communique

* LED cellulaire
	* Connexion au réseau mobile établie

### GL-X2000

![gl-x2000 led](https://static.gl-inet.com/docs/router/en/4/faq/led/gl-x2000_led.jpg){class="glboxshadow"}

* LED d'alimentation
	* Vert : sous tension
	* Éteinte : hors tension

* LED Internet
	* Allumée : Internet est connecté
	* Éteinte : Internet est déconnecté

* LED Wi-Fi 2.4G
	* Allumée : le Wi-Fi 2.4G est prêt
	* Clignotante : le Wi-Fi 2.4G communique

* LED Wi-Fi 5G
	* Allumée : le Wi-Fi 5G est prêt
	* Clignotante : le Wi-Fi 5G communique

* LED cellulaire
	* Connexion au réseau mobile établie

### GL-X750-V2

![gl-x750v2 led](https://static.gl-inet.com/docs/router/en/4/faq/led/x750v2_led.jpg){class="glboxshadow"}

* LED d'alimentation
	* Vert : sous tension
	* Éteinte : hors tension

* LED Internet
	* Allumée : Internet est connecté
	* Éteinte : Internet est déconnecté

* LED Wi-Fi 5G
	* Allumée : le Wi-Fi 5G est prêt
	* Clignotante : le Wi-Fi 5G communique

* LED Wi-Fi 2.4G
	* Allumée : le Wi-Fi 2.4G est prêt
	* Clignotante : le Wi-Fi 2.4G communique

* LED cellulaire
	* Connexion au réseau mobile établie

### GL-XE300

![gl-xe300 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/gl-xe300_led.png){class="glboxshadow gl-70-desktop"}

* LED de batterie (pendant la charge)
	* 1/4 clignotant : niveau de batterie 0 %-25 %
	* 2/4 clignotants : niveau de batterie 26 %-50 %
	* 3/4 clignotants : niveau de batterie 51 %-75 %
	* 4/4 clignotants : niveau de batterie 76 %-99 %
	* 4/4 fixes : niveau de batterie 100 %
	
* LED de batterie (hors charge)

	Lorsqu'il n'est pas en charge, le voyant de batterie s'allume pendant 5 secondes après avoir appuyé sur le bouton d'alimentation.

	* 1/4 fixe : niveau de batterie 0 %-20 %
	* 2/4 fixes : niveau de batterie 21 %-50 %
	* 3/4 fixes : niveau de batterie 51 %-75 %
	* 4/4 fixes : niveau de batterie 76 %-100 %

* LED d'alimentation
	* Vert : sous tension
	* Éteinte : hors tension

* LED WAN
	* Allumée : le port WAN est connecté
	* Clignotante : le port WAN communique
	* Éteinte : le port WAN est déconnecté

* LED LAN
	* Allumée : le port LAN est connecté
	* Clignotante : le port LAN communique
	* Éteinte : le port LAN est déconnecté

* LED Wi-Fi
	* Allumée : le Wi-Fi 2.4G est prêt
	* Clignotante : le Wi-Fi 2.4G communique

* LED cellulaire
	* Connexion au réseau mobile établie

## Wi-Fi domestique

### GL-BE9300

![gl-be9300 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/be9300_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-BE6500

![gl-be6500 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/be6500_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-MT6000

![gl-mt6000 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/gl-mt6000_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-AX1800

![gl-ax1800 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/gl-ax1800_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

### GL-B3000

![gl-b3000 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/b3000_interface.jpg){class="glboxshadow"}

- Bleu fixe : démarrage du système
- Bleu clignotant lentement : recherche du réseau
- Bleu clignotant rapidement : mise à niveau du firmware
- Blanc fixe : fonctionnement normal

## Passerelles de sécurité et mini-routeurs

### GL-MT5000

![gl-mt5000 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/mt5000_interface.png){class="glboxshadow"}

* LED système
	* Bleu : démarrage du système
	* Bleu clignotant : recherche du réseau
	* Blanc : fonctionnement normal
* LED VPN
	* Éteinte : le client VPN n'est pas connecté
	* Clignotante : le client VPN est en cours de connexion
	* Fixe : le client VPN est connecté
	
Remarque : l'état du serveur VPN n'est pas affiché sur la LED.

### GL-MT2500/GL-MT2500A

![gl-mt2500 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/mt2500_interface.jpg){class="glboxshadow"}

* LED système
	* Bleu : démarrage du système
	* Bleu clignotant : recherche du réseau
	* Blanc : fonctionnement normal
* LED VPN
	* Éteinte : le client VPN n'est pas connecté
	* Clignotante : le client VPN est en cours de connexion
	* Fixe : le client VPN est connecté
	
Remarque : l'état du serveur VPN n'est pas affiché sur la LED.

### GL-MT300N-V2

![gl-mt300n-v2 interface](https://static.gl-inet.com/docs/router/en/4/faq/led/mt300n-v2_interface.png){class="glboxshadow"}

- LED d'alimentation
	- Allumée : sous tension
	- Éteinte : hors tension

- LED centrale (configurable)
	- Allumée : cette LED a été personnalisée
	- Éteinte : cette LED n'a pas été personnalisée (mode par défaut)

- LED sans fil
	- Allumée : le Wi-Fi 2.4G est activé
	- Éteinte : le Wi-Fi 2.4G est désactivé
	- Clignotante : le Wi-Fi 2.4G est connecté

	Pour les premiers lots, la LED sans fil peut s'afficher en rouge au lieu de blanc en fonctionnement normal.

### Série GL-AR300M

![gl-ar300m interface](https://static.gl-inet.com/docs/router/en/4/faq/led/ar300m_interface.png){class="glboxshadow"}

- LED d'alimentation
	- Allumée : sous tension
	- Éteinte : hors tension

- LED centrale (configurable)
	- Allumée : cette LED a été personnalisée
	- Éteinte : cette LED n'est pas personnalisée (mode par défaut)

- LED sans fil
	- Allumée : le Wi-Fi 2.4G est connecté
	- Éteinte : le Wi-Fi 2.4G est déconnecté

	Pour les premiers lots, la LED sans fil peut s'afficher en rouge au lieu de blanc en fonctionnement normal.
