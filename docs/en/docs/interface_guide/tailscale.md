...
Tailscale is a VPN service that makes the devices and applications you own accessible anywhere in the world, securely and effortlessly. For more information about Tailscale, please visit [Tailscale official website](https://tailscale.com/){target="_blank"}.
...
6. Disable key expiry. 
    ...
    As a security feature, users need to periodically reauthenticate on each of their devices. To avoid connectivity interruptions when the exit node authentication key expires, we recommend disabling key expiry for your exit node. Click [here](https://tailscale.com/docs/features/access-control/key-expiry){target="_blank"} for more details about key expiry.
...
8. Test connectivity.
    ...
    3. Disconnect the exit node for a comparative test.
        ...
        Open a new browser tab and visit [ipcheck.ing](https://ipcheck.ing/){target="_blank"} or any IP lookup service. It will now show your laptop's native public IP address, proving the device is using your local internet connection instead (Boston, in this example).
...
For more information, you can refer to [Exit Nodes (route all traffic)](https://tailscale.com/kb/1103/exit-nodes/){target="_blank"}.
...
IP Masquerading is recommended for the following scenarios:

- If you want to simplify routing through a subnet router.

- If you want to skip configuring return routes on the destination network.
...
After configuring, if your devices are connected to the router, they will route their traffic through the exit node to access the Internet, and all your Internet traffic will appear to originate from the Exit Node's location.
