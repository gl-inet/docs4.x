# Guida utente di Mango (GL-MT300N-V2)

## Panoramica del prodotto

Mango (GL-MT300N-V2) e un mini router tascabile progettato per essere portatile e adatto ai viaggi, con supporto a velocita di trasmissione wireless fino a 300 Mbps. Mango offre funzioni di sicurezza avanzate, tra cui supporto per OpenVPN, WireGuard e un server DNS. Non solo consente di convertire una rete pubblica in una rete Wi-Fi privata per una navigazione piu sicura, ma permette anche di caricare file di configurazione VPN da oltre 30 provider VPN e di usarlo come client VPN, aggiungendo un ulteriore livello di privacy e sicurezza tramite traffico crittografato tra il dispositivo e il server VPN.
...
## Come configurare Mango

Per configurare Mango, puoi usare uno dei quattro metodi di connessione Internet supportati: Ethernet, Repeater, Tethering e Cellular. Segui i passaggi riportati di seguito.

### 1. Accensione

Collega il cavo di alimentazione Micro USB alla porta di alimentazione del router, quindi collega l'altra estremita a un alimentatore 5V/2A, non incluso, e inseriscilo in una presa elettrica.
...
- Wi-Fi
    ...
    Sul dispositivo, vai in Settings -> WLAN, individua il nome della rete Wi-Fi del router nell'elenco delle reti disponibili e inserisci la password. Il nome di rete e la password predefiniti sono stampati sull'etichetta inferiore del router.
...
Tieni presente che, se modifichi le informazioni Wi-Fi, dovrai ricollegare il dispositivo alla rete Wi-Fi del router usando le credenziali aggiornate.
...
Configura Mango usando uno dei metodi di connessione Internet supportati: Ethernet, Repeater, Tethering e Cellular. Se desideri usare la funzione [Multi-WAN](../../interface_guide/multi-wan.md), configura piu di una connessione Internet.
...
=== "Cellular"
    ...
    Per istruzioni dettagliate, fai riferimento a [Collegarsi a Internet tramite modem USB](../../interface_guide/internet_cellular.md).

## Come configurare una VPN

Una VPN crea traffico sicuro e crittografato tra il dispositivo e il server VPN. Offre un ulteriore livello di privacy e sicurezza come client VPN e consente di accedere a una rete remota come server VPN. Mango, come gli altri router GL.iNet, supporta OpenVPN e WireGuard.

=== "OpenVPN"

    Mango, come gli altri router GL.iNet, supporta il protocollo OpenVPN, che offre un elevato livello di sicurezza. Per configurare OpenVPN, segui questi tutorial:
...
=== "WireGuard"
    ...
    Mango, come gli altri router GL.iNet, supporta il protocollo WireGuard, che offre ottima velocita e praticita. Per configurare WireGuard, segui questi tutorial:
...
=== "Firewall"
    ...
    * DMZ (Demilitarized Zone): colloca un dispositivo all'esterno del firewall principale, permettendogli di avere accesso illimitato a Internet mentre il resto della rete resta protetto da potenziali minacce.
    ...
    Per configurare le impostazioni relative al firewall, fai riferimento a [Firewall](../../interface_guide/firewall.md).
...
=== "LAN"
    ...
    * Router IP Address: modifica l'indirizzo IP del router per adattarlo meglio alla configurazione della rete.
    * Netmask: imposta la subnet mask della rete, che determina dimensioni e intervallo degli indirizzi IP.
    * DHCP: abilita o configura il Dynamic Host Configuration Protocol, che assegna automaticamente indirizzi IP ai dispositivi della rete.
...
=== "MAC Address"
    ...
    La pagina MAC Address consente di visualizzare e gestire gli indirizzi MAC associati al router. Le funzioni principali disponibili in questa pagina includono:

    * Factory Default: mostra gli indirizzi MAC predefiniti per le modalita Ethernet e Repeater del router, fornendo un riferimento per le impostazioni hardware originali.
    ...
    * Manual: consente di specificare manualmente un indirizzo MAC personalizzato per il router. Inoltre, puoi usare il pulsante Random per generare un indirizzo MAC casuale, offrendo maggiore flessibilita e privacy.
    ...
    Per gestire l'indirizzo MAC, fai riferimento a [MAC Address](../../interface_guide/mac_address.md).
...
=== "Admin Password"
    ...
    La pagina Admin Password consente di impostare o modificare la password dell'interfaccia amministrativa del router, garantendo che solo gli utenti autorizzati possano accedere e modificare le impostazioni del router. Questa password e fondamentale per mantenere sicurezza e integrita della rete, proteggendo da accessi non autorizzati e modifiche di configurazione.
