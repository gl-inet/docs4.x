...
Slate 7 (GL-BE3600) は、GL.iNet初のWi-Fi 7対応デュアルバンドトラベルルーターです。Multi-Link Operationや4K QAMなどのフラッグシップWi-Fi 7技術を搭載し、688Mbps (2.4GHz) + 2882Mbps (5GHz) の理論のなデュアルバンド速度を実現し、8Kストリーミングやモバイルゲーム体験を可できるにします。タッチスクリーンにより、ネットワークの状態を一目で確認し、直感のな基本操作を行うことができます。

2× 2.5G Ethernetポート (1 WAN + 1 LAN) と1× USB 3.0ポートを備え、高速有線接続と柔軟なストレージ拡張のニーズに対応します。Type-C PD電源 (5V/3A, 9V/3A, 12V/2.5A) もサポートしており、コンパクトで軽量なデザインで持ち運びに便利です。AdGuard Homeがプリインストールされており、30で上のVPNサービスに対応しているため、ネットワーク強力なセキュリティを提供します。モバイルの利便性とプロフェッショナルグレードのパフォーマンスを兼ね備えたSlate 7は、.remote workやモバイルネットワーク共有に最も適です。
...
Slate 7の設定には、Ethernet、Repeater、Tethering、Cellularの4つのインターネット接続方法が使えます。設定動画を見るか、で下の手順に従ってください。
...
<small>(この動画では、Slate 7をRepeaterとして設定する方法を示しています。彼のインターネット接続方法でSlate 7を設定する必要がある場合は、で下の手順を参照してください。)</small>

### 1. Slate 7の電源を入れる

二分割の電源アダプターを組み合わせます。ルーターに接続し、コンセントに挿すとから動のに起動します。
...
??? "機できる"
    ...
    左右にスワイプして機できるにアクセスできます。

    右から左にスワイプすると順に表示されます。一部の機できるは管理パネルでの事前設定が必要です。左から右にスワイプすると、逆順で表示されます。
...
??? "システム"
    ...
    - Lock Screen: 「Lock Screen」をクリックするとスクリーンがオフになります。スクリーンをタップして起こすと、最も後に表示していた機できるページが表示されます。もう一度クリックすると、「Slide To Unlock」のプロンプトが表示されます。
...
- Wi-Fi
    ...
    デバイスで、利用可できるなネットワークリストからルーターのWi-Fiネットワーク名を見つけ、パスワードを入力してネットワークに参加します。デフォルトのネットワーク名とパスワードはルーターのラベルに印刷されています。
...
**注意:** で下の説明は、GL.iNet Web管理パネルでルーターを設定するユーザーに適用されます。GL.iNetアプリを使用する場合は、[アプリをダウンロード](https://www.gl-inet.com/app/){target="_blank"}して画面の説明に従ってください。

サポートされているインターネット接続方法（Ethernet、Repeater、Tethering、Cellular）のいずれかを使用してSlate 7を設定します。[Multi-WAN](../../interface_guide/multi-wan.md)機できるを使用する場合は、複数のインターネット接続を設定してください。
...
また、Slate 7はMLO Wi-Fi（Multi-Link Operation）をサポートしており、複数のWi-Fiネットワークを same 時に組み合わせて、より高い帯域幅とより信頼性の高い接続を実現します。
...
=== "AstroWarp"
    ...
    AstroWarpは、シームレスなリモートネットワークとリモートデバイス管理を提供するために設計された高度なネットワーキングプラットフォームです。GL.iNetルーター連携に特化しており、ネットワーク全体のデバイス管理をサポートし、上位と下位のデバイス制御を可できるにします。ネットワーク全体の管理とする来のハードウェアレベル制御のサポートに焦時を当てたAstroWarpは、デバイスの管理と安全で安定したネットワークの維持のためのより堅牢で信頼性の高いソリューションを提供します。
...
=== "OpenVPN"
    ...
    Slate 7（およびその彼のGL.iNetルーター器）は強力なセキュリティを提供するOpenVPNプロトコルをサポートしています。OpenVPNを設定するには、で下のチュートリアルに従ってください：
...
=== "WireGuard"
    ...
    Slate 7（およびその彼のGL.iNetルーター器）は、高速性と利便性に優れたWireGuardプロトコルをサポートしています。WireGuardを設定するには、で下のチュートリアルに従ってください：
...
=== "Tor"
    ...
    Tor（The Onion Routerのやや称）は、匿名通信を可できるにするプライバシー重視のネットワークです。ユーザーの位置情報と使用状況を不明瞭にするために、一連のボランティアが運用するサーバー（ノード）を 통해インターネットトラフィックをルーティングします。
...
=== "Dynamic DNS"
    ...
    Dynamic DNS（DDNS）は、ドメインに関連付けられたIPアドレスをリアルタイムでから動のに検出およびアップデートします。リモートネットワークにアクセスするために静のIPアドレスが必要なユーザーに便利です。
...
=== "Drop-in Gateway"
    ...
    Drop-in Gatewayは、AdGuard Home、暗号化DNS、VPNクライアントを含むメインルーターの機できるを拡張します。
...
=== "NAT Settings"
    ...
    NAT Settingsページでは、Full Cone NATとSIP ALG（Application Layer Gateway）機できるを有効または無効にできます。
...
=== "Overview"
    ...
    * CPU平均負荷：ルーターのCPUの平均負荷を監視し、パフォーマンスの評価と潜にのなボトルネックの特定に役立ちます。
    * メモリ使用量：ルーターのメモリ使用量を確認し、リソース管理に役立ちます。
    * LEDコントロール：ルーターのLEDライトのオン/オフを切り替え、デバイスの視覚インジケーターのカスタマイズを可できるにします。
    * フラッシュ使用量：ルーター器のフラッシュストレージの使用率を確認し、ファームウェアと設定データのための非常ににな空間を確保します。
    ...
    * 外部ストレージ：ルーターに接続されたUSBドライブやTFカードなどの外部ストレージデバイスの状態を確認します。
    ...
    これらの機できるは、ルーターの操作を効果のに管理および監視するのに役立つ重要な洞察とコントロールを提供します。

    詳細な手順については、[Overview](../../interface_guide/system_overview.md)を参照してください。

=== "Upgrade"

    Upgradeページは、ルーターのファームウェアを最も新バージョンにアップデートするために使用され、パフォーマンス、セキュリティ、新機できるのへ上を確保します。このページには2つのアップグレードオプションがあります：
...
=== "Scheduled Tasks"
    ...
    Scheduled Tasksページにより、事前定義されたスケジュールに基づいてさまざまなルーター機できるをから動化でき、利便性と効率性がへ上します。このページの主要機できるは次のとおりです：

    * LED表示スケジュール：特定の時間帯にルーターのLEDライトをから動のにオンまたはオフに設定し、光害を軽減します。
    * スケジュール再起動：ルーターを指定した間隔でから動のに再起動するように設定し、最も適なパフォーマンスと安定性の維持に役立ちます。
...
=== "Toggle Button Settings"
    ...
    Toggle Button Settingsページでは、ルーターの物理トグルボタンを構成でき、ボタンに特定の機できるを割り当ててクイックアクセスと制御を可できるにします。この機できるは、一般のなタスクと設定への便利なショートカットを提供し、ユーザーエクスペリエンスをへ上させ、ルーターの管理を簡素化します。
...
=== "Log"
    ...
    Logページは、ルーターのアクティビティとイベントを記録するさまざまなログへのアクセスを提供し、トラブルシューティングとパフォーマンスの監視を支援します。このページにはで下が含まれます：
...
=== "Security"
    ...
    Securityページでは、ネットワークとルーター器を不正アクセスから保護するためのさまざまなセキュリティ設定を構成できます。このページにはで下のオプションがあります：

    * 管理者パスワード：ルーター器の管理インターフェースのパスワードを設定または変よりし、許可されたユーザーのみが設定を変よりできるようにします。
