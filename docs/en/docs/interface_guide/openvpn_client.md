# Set Up OpenVPN Client on GL.iNet Routers
...
To set up OpenVPN client on a GL.iNet router, watch this video or refer to the steps below.
...
Generally, you need to visit the official website of the VPN service provider you subscribed to first, download the configuration file, then upload it to your router to set it up as an OpenVPN client. If you don't know how to get the configuration file, refer to [this link](#get-configuration-files-from-openvpn-service-providers) or contact their support.

You can set up an OpenVPN client via the web admin panel or [mobile app](../faq/mobile_app.md). Below are steps to set up via the web admin panel.

---

In the web Admin Panel, navigate to **VPN** -> **OpenVPN Client**. 

If you have a NordVPN subscription, click **NordVPN** to log in; otherwise, click **Add Manually** to upload the OpenVPN configuration files.
...
1. Log in to your NordVPN web account [here](https://my.nordaccount.com/){target="_blank"}.
    ...
    After login, on the Nord Dashboard, click **NordVPN** on the left menu, then click **Set up NordVPN manually**.
    ...
    You will find the **service credentials**. Verify your email first to get your credentials, then copy them for later use.
...
2. Log in to your router's web admin panel, and go to **VPN** -> **OpenVPN Client** -> **NordVPN**. 

    Input the **service credentials** obtained in step 1 (Note: The service credentials is **NOT** your login account email and password), then click **Save and Continue**.
...
??? "ExpressVPN"
    ...
    2. On the left navigation bar, click **Set up your devices**.
    ...
    3.  Under **Applications**, expand **Advanced setup** and click **Manual Configuration**.
...
??? "Hotspot Shield"
    ...
    **Note: The Hotspot Shield Router configuration files are no longer available or supported. The following steps are here for legacy purposes for those that may still have the files installed.**
    ...
    1. Go to [https://www.hotspotshield.com/](https://www.hotspotshield.com/){target="_blank"} and click on Account. Sign in if you're asked.
    ...
    2. Go to [https://app.hotspotshield.com/app/hotspotshield/router](https://app.hotspotshield.com/app/hotspotshield/router){target="_blank"}
...
??? "IPVANISH"
    ...
    - You can also download individual server configuration files [here](https://www.ipvanish.com/software/configs/){target="_blank"}, but you will need to download **ca.ipvanish.com.crt** as well. Before uploading to the router, you need to compress the **ca.ipvanish.com.crt** and .ovpn configuration files into a .zip archive.
...
??? "PrivateVPN"
    ...
    [Download](https://static.gl-inet.com/docs/router/en/4/interface_guide/openvpn_client/privatevpn/PrivateVPN-TUN.zip){target="_blank"} directly.
...
??? "PureVPN"
    ...
    1. [Sign in to your PureVPN account.](https://my.purevpn.com/){target="_blank"}
...
??? "SaferVPN"
    ...
    Click [here](https://support.safervpn.com/hc/en-us/articles/360035425314-What-are-SaferVPN-s-OpenVPN-configuration-ovpn-files-for-manual-setup){target="_blank"} to download configuration files directly.
