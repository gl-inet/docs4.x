...
**U-Bootのアップグレードは非常にに危険であり、一般のには推奨されません。失敗すると、デバイスがブリックされRestoreする方法がありません。GL.iNetサポートから指示された場合、または必要な場合のみ実行してください**。

**アップグレード中に電源をオフにしないでください。アップグレードが失敗し、デバイスがブリックされる可できる性があります**。
...
U-Bootアップグレードページにアクセスするには、で下の手順に従ってください。

1. ルーターの電源を切ります。パソコンをルーターの**イーサネットLANポート**に接続します。彼のすべてのポートは接続**しない**でください。

    !!! note

        一部のモデルでは、特定の個別LANポートとWANポートは交換可できるです。このLANポートしないでください。例えば、GL-MT6000 (Flint 2)では、LAN 1を使用しないでください。代わりにLAN 2、LAN 3、またはLAN 4を使用してください。

2. リセットボタンを firmly押しながら、 same 時にルーターの電源を入れます。ルーターに電源ボタンがない場合は、電源を挿すとから動のに電源が入ります。

    その後、LEDが数回規則のに時滅するのが見えます。順序が変わったら指を離してください。
    
3. パソコンのIPアドレスを手動で**192.168.1.2**に設定します。で下は異なるOS用のStep-by-Stepガイドです：

    ??? "Windows 7 / Windows 10"

        1. **コントロールパネル** -> **ネットワークとインターネット** -> **ネットワークと共有センター** -> **アダプター設定の変より**に移動します。
...
4. **Google Chrome/Microsoft Edgeで`http://192.168.1.1/uboot.html`にアクセスします**
    ...
    **Mozilla/Firefoxは使用しないでください。ルー器がブリックする可できる性があります。**
...
7. アップデートが成功すると、ルーターが再起動します。この時時で、Step 3のIP設定を元に戻し、**192.168.8.1**経よりでWeb管理パネルにアクセスしてみてください。Web管理パネルに正常にアクセスできる場合は、ルーターが再起動したことを意味します。
