# Cannot connect to a WireGuard server with obfuscation enabled

VPN obfuscation is a technique that disguises VPN traffic to look like regular internet traffic. Currently, some advanced GL.iNet routers support the AmneziaWG protocol, allowing you to set up a WireGuard server with traffic obfuscation enabled.

If you fail to establish a connection to a WireGuard server with obfuscation enabled, follow the steps below to identify to troubleshoot.

1. **Ensure the VPN configuration file imported on your client device is the exact file exported from the GL.iNet WireGuard server.**

    A single WireGuard configuration file is uniquely bound to one client device and cannot be used simultaneously on multiple client devices. Using the same configuration across multiple clients will cause connection failures.
...
2. **Match AmneziaWG protocol versions on your server and client.**
    ...
    ??? note "Check via Router Web Admin Panel"
        ...
        2. Navigate to **VPN** > **WireGuard Server** > **Configuration** and check the obfuscation parameters. If the configuration contains **S3-S4**, and **H1–H4** are variable ranges (rather than fixed values), this router is running AmneziaWG 2.0, as shown below. 
        
            ![awg 2.0 web](https://static.gl-inet.com/docs/router/en/4/faq/cannot_connect_to_wgserver_with_obfuscation_enabled/awg_2.0_web.png){class="glboxshadow"}

            Otherwise, it uses AmneziaWG 1.0, as shown below.

            ![awg 1.0 web](https://static.gl-inet.com/docs/router/en/4/faq/cannot_connect_to_wgserver_with_obfuscation_enabled/awg_1.0_web.png){class="glboxshadow"}
    ...
    ??? note "Check via SSH Command"
        ...
        2. Run `opkg list|grep awg` and check the suffix of the **kmod-amneziawg** package in the output. If marked with **2.0**, the router supports AmneziaWG 2.0, as shown below.
            ...
            ![awg 2.0 ssh](https://static.gl-inet.com/docs/router/en/4/faq/cannot_connect_to_wgserver_with_obfuscation_enabled/awg_2.0_ssh.png){class="glboxshadow"}

            Otherwise, it runs AmneziaWG 1.0, as shown below.

            ![awg 1.0 ssh](https://static.gl-inet.com/docs/router/en/4/faq/cannot_connect_to_wgserver_with_obfuscation_enabled/awg_1.0_ssh.png){class="glboxshadow"}
