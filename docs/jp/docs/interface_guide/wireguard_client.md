...
別の WireGuard サービスプロバイダーを利用している場合は、プロバイダーの Web サイトから設定ファイルをダウンロードし、以下の手順で WireGuard Client を設定してください。設定ファイルのダウンロード方法が分からない場合は、[このガイド](../tutorials/how_to_get_configuration_files_from_wireguard_service_providers.md) を参照するか、プロバイダーのサポートへお問い合わせください。
