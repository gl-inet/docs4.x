...
Beryl 7 (GL-MT3600BE) は、ビジネス旅行や休暇などのモバイルシナリオへけに設計されたWi-Fi 7対応デュアルバンドトラベルルーターです。Beryl AXのアップグレード版として、Multi-Link Operation (MLO)や4K QAMなどのWi-Fi 7技術をサポートし、理論のなデュアルバンド速度は688Mbps (2.4GHz) + 2882Mbps (5GHz) — 8Kストリーミングやモバイルゲームなどの高速ニーズに対応できます。

MediaTekクアッドコアプロセッサーで動作し、512MB NANDフラッシュストレージを備え安定したマルチタスクとさまざまなOpenWrtプラグインとの互換性を確保します。2x 2.5G Ethernetポート (1 WAN + 1 LAN) と1x USB 3.0ポートを備え、高速有線接続とストレージ拡張の両方のニーズに対応します。PD互換性により、標準のスマートフォン充電器のーブルで電源を供給できるため、荷物を軽くできます。120で上のデバイス same 時接続をサポートしており、移動中の生活に最も適なポータブルデザインが特徴です。
...
Beryl 7の設定には，Ethernet、Repeater、Tethering、Cellularの4つのインターネット接続方法が使えます。設定動画を見るか、で下の手順に従ってください。
...
二分割の電源アダプターを組み合わせます。ルーター者に接続し，コンセントに挿すとから動のに起動します。

### 2. Beryl 7に接続する

Wi-FiまたはEthernetを使用してコンピュータまたはモバイルデバイスをルーター者に接続します。
...
- Wi-Fi
    ...
    デバイスで、Settings -> WLANを開き，利用可できるなネットワークリストからルーター者のWi-Fiネットワーク名を見つけ、パスワードを入力してネットワークに参加します。（デフォルトのネットワーク名とパスワードはラベルに印刷されています。）
...
**注意:** で下の説明は、GL.iNet Web管理パネルでルーター者を設定するユーザーに適用されます。GL.iNetアプリを使用する場合は，[アプリをダウンロード](https://www.gl-inet.com/app/){target="_blank"}して画面の説明に従ってください。

サポートされているインターネット接続方法（Ethernet、Repeater、Tethering、Cellular）のいずれかを使用してBeryl 7を設定します。[Multi-WAN](../../interface_guide/multi-wan.md)機できるを使用する場合は、複数のインターネット接続を設定してください。
...
=== "Ethernet"
    ...
    Beryl 7のWANポートをEthernet cableで上游デバイス（モデムなど）に接続します。
...
また、Beryl 7はMLO Wi-Fi（Multi-Link Operation）をサポートしており、複数のWi-Fiネットワークを same 時に組み合わせて、より高い帯域幅とより信頼性の高い接続を実現します。
...
=== "OpenVPN"
    ...
    Beryl 7（およびその彼のGL.iNetルーター者）は強力なセキュリティを提供するOpenVPNプロトコルをサポートしています。OpenVPNを設定するには、で下のチュートリアルに従ってください：
...
=== "WireGuard"
    ...
    Beryl 7（およびその彼のGL.iNetルーター者）は、高速性と利便性を備えたWireGuardプロトコルをサポートしています。WireGuardを設定するには、で下のチュートリアルに従ってください：
...
=== "Drop-in Gateway"
    ...
    Drop-in Gatewayは、AdGuard Home、暗号化DNS、VPNクライアントを含むメインルーター者の機できるを拡張します。
...
=== "Toggle Button Settings"
    ...
    Toggle Button Settingsページでは、ルーター者の物理トグルボタンを構成でき、ボタンに特定の機できるを割り当ててクイックアクセスと制御を可できるにします。この機できるは、一般のなタスクと設定への便利なショートカットを提供し、ユーザーエクスペリエンスをへ上させ、ルーター者の管理を簡素化します。
...
=== "Security"
    ...
    Securityページでは、ネットワークとルーター者を不正アクセスから保護するためのさまざまなセキュリティ設定を構成できます。このページにはで下のオプションがあります：

    * 管理者パスワード：ルーター者の管理インターフェースのパスワードを設定または変よりし、許可されたユーザーのみが設定を変よりできるようにします。
