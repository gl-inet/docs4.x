# Guide d'utilisation de Spitz AX (GL-X3000)

## Aperçu du produit

Spitz AX (GL-X3000) est une passerelle cellulaire Wi‑Fi 6 double SIM conçue pour fournir des connexions rapides et fiables, en particulier dans les zones éloignées et lors des déplacements sur la route. Elle propose quatre méthodes d'accès à Internet : Cellular (cartes SIM), Ethernet, Repeater et Tethering. Elle prend en charge le Multi-WAN (conmutation par erreur et équilibrage de charge), les VPN (OpenVPN et WireGuard), le contrôle parental, AdGuard Home, la redirection de ports, Tailscale et bien plus encore.
...
| -------------------------------------- | ---------------------- | ---------------------- | ---------------------- | ---------------------- | ---------------------- |
| Internet connecté (Cellular actif)   | Vert allumé            | Vert allumé            | Vert allumé            | Vert allumé            | Vert allumé            |
| Internet connecté (Cellular inactif) | Vert allumé            | Vert allumé            | Vert allumé            | Vert allumé            | Éteint                 |
| Pas de connexion Internet            | Vert allumé            | Éteint                 | Vert allumé            | Vert allumé            | Éteint                 |
| Mise à jour du firmware              | Vert allumé            | Clignote toutes les 0,5 s | Clignote toutes les 0,5 s | Clignote toutes les 0,5 s | Vert allumé         | 
| Réinitialisation réseau (appuyez sur reset < 3 s) | Clignote toutes les 1 s | Vert allumé | Vert allumé | Vert allumé | Vert allumé |
| Réinitialisation d'usine (appuyez sur reset pendant 10 s) | Clignote toutes les 0,25 s | Vert allumé | Vert allumé | Vert allumé | Vert allumé | 

**Conseil :** lorsqu'il est connecté à Internet, un voyant LED 2,4 GHz ou 5 GHz clignotant signifie que des appareils Wi‑Fi sont connectés et transmettent activement des données.

## Comment configurer Spitz AX
...
Configurez votre Spitz AX en utilisant l'une des méthodes de connexion Internet prises en charge : Cellular, Ethernet, Repeater et Tethering. Si vous souhaitez utiliser la fonction [Multi-WAN](../../interface_guide/multi-wan.md), veuillez configurer plus d'une connexion Internet.

=== "Cellular"

    ![cellular](https://static.gl-inet.com/docs/router/en/4/user_guide/gl-x3000/internet/x3000_cellular.jpg){class="glboxshadow"}

    Si vous avez déjà inséré la carte SIM dans votre routeur, la connexion Internet devrait s'établir automatiquement. (Vous devriez voir le nom de votre opérateur SIM et un point vert apparaître dans la section Cellular.)
    
    Sinon, cliquez sur l'option **Auto Setup** si elle apparaît.
    ...
    Découvrez [ici](../../tutorials/how_to_use_esim_physical_card_with_glinet_routers.md) comment utiliser la carte physique eSIM sur un routeur GL.iNet.
...
=== "WireGuard"
    ...
    Spitz AX (comme les autres routeurs GL.iNet) prend en charge le protocole WireGuard, qui offre d'excellentes vitesses et une grande simplicité d'utilisation. Pour configurer WireGuard, suivez ces tutoriels :
...
=== "Port Ethernet"
    ...
    La page Port Ethernet vous permet de configurer les ports WAN et LAN, de définir l'interface WAN/LAN sur Ethernet, de spécifier le mode MAC et l'adresse MAC de l'interface WAN, et d'afficher le débit négocié du port réseau.
...
=== "Tâches planifiées"
    ...
    * Planification de l'état du Wi‑Fi 5 GHz : définissez un planning pour contrôler la bande Wi‑Fi 5 GHz, afin de mieux gérer la disponibilité du réseau et la consommation d'énergie.
