...
2. Reset ボタンを **10 秒間**長押ししてから離すと、ハードリセットが実行されます。押している間は画面上の案内を確認し、指示に従って操作してください。

    これにより、すべての設定が削除されます。十分ご注意ください。

**ヒント**: 工場出荷時設定へのリセットを行わずにネットワーク接続だけを復旧したい場合は、Reset ボタンを **4 秒間**長押ししてから離し、ネットワーク修復を実行してください。押している間は画面上の案内を確認し、指示に従って操作してください。

これにより、Wi-Fi 設定、VPN 設定（無効）、システム設定などを保持したまま、ネットワークインターフェースが再起動します。

!!! note "Soft Reset"

    1. <u>ソフトリセット</u> はデータを消去せずにデバイスを再起動します。一方、<u>工場出荷時設定へのリセット</u> はデフォルト設定に戻し、すべてのユーザーデータを削除します。

    2. Wi-Fi が無効になっている場合、ソフトリセットにより Wi-Fi はデフォルトの有効状態に戻ります。

    3. ソフトリセットは、デバイスを非ルーターモードからルーターモードへ素早く切り替える用途にも使用できます。
